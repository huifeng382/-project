...
def describe_target(y, name):
    ...
    for k, v in stats.items():
        print(f"  {k}: {v:.4e}" if isinstance(v, float) and abs(v) > 1e-10 else f"  {k}: {v}")
    return stats
...
top_pearson_idx = np.argsort(np.abs(pearson_corr))[::-1][:10]
