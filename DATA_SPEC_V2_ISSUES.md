...
1. **总量包含已交付数据**：规格 ~4,000 expr / ~5 万电路 / ~60 万行是 V2 **累计**目标，**包含**已交付的
   `batch_v2_full`（579 expr / 8,679 电路 / 69,432 行）+ `batch_v2_io`（389 expr / 4,248 电路 / 57,764 行）
   = **968 expr / 12,927 电路 / 127,196 行**（约规格 24%）。
2. **续产量 = ~3,032 expr**（4,000 − 968），expr 编号从 expr9000+ 之后继续递增，不与已交付重叠。
3. **续产必须带 I/O 覆盖（数量与 I/O 绑定，不可拆开）**：按 V2 规格分桶——输入 1~2/3~4/5~8/9~16 各 ~25%、
