...
>
> **范围说明**：生成**数量**问题在 V1.0/V2.0 按 2026-08-20 讨论暂缓；**V3.0（2026-08-25）起数量与 I/O 覆盖绑定、列为正式要求**（见第四节），
> 续产必须带 I/O 分桶覆盖，且总量口径**包含**已交付数据。
...
| **大项 ⏸** | **数量 + I/O 覆盖（绑定）** | **续产 ~3,032 expr（总量口径见第四节），必须带 I/O 分桶覆盖**（18 形状全盖、多输出 ≥20%、补 G1 缺失形状） | 总量 4,000 expr / ~50K 电路达标 |
...
### 4.3 总量口径与 I/O 覆盖要求（正式要求，取代原「暂缓」）

1. **总量包含已交付数据（已交付的 full + io 计入总数）**：规格 ~4,000 expr / ~5 万电路 / ~60 万行是 V2 **累计**目标，
   **明确计入**已交付的 `batch_v2_full`（579 expr / 8,679 电路 / 69,432 行）+ `batch_v2_io`（389 expr / 4,248 电路 / 57,764 行）
   = **968 expr / 12,927 电路 / 127,196 行**（约占规格 24%）。→ **续产只需补齐差额，不需要重生成已交付部分**。
2. **续产量 = ~3,032 expr**（4,000 − 968），expr 编号从 expr9000+ 之后继续递增，不与已交付重叠。
3. **续产必须带 I/O 覆盖（数量与 I/O 绑定，不可拆开）**：按第四节 4.4 的分配策略——输入分桶 ~20/25/25/30、
   输出 1/2/3+ 三档、多输出 ≥20%、**覆盖全部 18 种 benchmark 形状（含补 G1 缺失的 9入6出 等 ≥4 输出形状）**。
4. **full 与 io 的分工保留**：`batch_v2_full` 继续作为 4-pin 口径（已有 579 expr，4-pin 训练够用，可不扩）；
   续产以任意 I/O 口径为主（对齐 Rust 集成需求）；若 4-pin 训练需要更多样本，可在续产中按比例分出一部分 4-pin 组。

### 4.4 I/O 分配策略（续产生成指南，2026-08-25）
