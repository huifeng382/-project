...
### 阶段 1：尝试升级现有 60 万行到 v9（有前提条件，不强求）

1. 数据方先确认：原始 SPICE 瞬态波形文件（`.raw` / `.tr0` / 仿真输出）是否还保留？
2. **如果保留**→ 对已有 60 万行的 SPICE 波形做后处理，提取 v9 新增的 3 个晶体管字段（`ids_rise_time` / `vgs_swing` / `ids_charge`），补入原有的 `transistor_wave_json`。**零仿真成本**——同一组波形重新跑后处理脚本即可。
3. **如果波形文件已删除或无法找回**→ **不需要重新跑这 60 万行的仿真**。直接跳过阶段 1，进入阶段 2。旧 60 万行按 v8 格式（4 个 transistor 子字段）保留不动即可——后续 120 万新数据到位后，旧数据的边际贡献占比小。
4. 若升级成功，更新后的数据直接覆盖旧 batch 目录（或作为新目录，旧数据归档），交付附 `coverage_report.json`。
