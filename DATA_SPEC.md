...
生成 8 个 Parquet 文件（所有批次每行均须含 `transistor_wave_json`，不再单设 batch_wave）：

```
# 第一批：手选电路全 sweep
data/batch1/circuit_static.parquet    # 200个电路
...
| 引脚要求 | 全部为4引脚（a,b,c,d） |
| 每电路corner数 | 9（3 slew × 3 load 全交叉） |
| 每corner组合 | 4个引脚 × 2个方向 × 2个vector = 16行（与第二批一致，**不降 vector**——保证每变体有足够 stimulus 行来降聚合噪声）|
| 每电路总行数 | 9 × 16 = 144行 |
| 第三批总行数 | 约57,600-72,000行 |
...
2. **所有批次每行都必须包含 `transistor_wave_json` 列**（方案 B，**绝无例外**）。受第七节完整性铁律约束：①列非空 100%；②四个子字段（gate/ids_avg/ids_peak/vds_swing）均 100% 有效；③交付附 `data/coverage_report.json` 报告所有批次覆盖率。
3. **方案 A（LIB 查表）数据已提供**：`data/std_cells.lib` + `data/sc_expansion.json` 已就位。优先度低于方案 B，方案 B 到位后可叠加。
4. `data/batch_wave/` 目录不再需要——`transistor_wave_json` 不再是独立批次专属列，而是所有批次的通用必填列。
...
| 项目 | v7 | v8 | 原因 |
|------|------|------|------|
| transistor_wave_json 范围 | 独立 batch_wave/ 20 电路（5.7% 总样本） | **所有批次（batch1/1b/2/3 全部）每一行都必须包含**（100% 总样本） | 信噪比诊断：预测噪声 RMS 17ps >> <2% 信号 0.48ps（SNR 0.11）。降噪需全量数据，5.7% 覆盖几无效果。零额外仿真成本（SPICE 已算过，仅提取写出）|
| batch_wave 目录 | 需要 | **不再需要，取消** | 该列已通用化，不再独立为一批 |
| 目标 | "突破 15% 误差瓶颈"（旧点精度口径） | **排序指标**（Spearman→1, 遗憾→0, <2% 成对分辨从 52%→70-75%）| 任务已重新定义为等价变体择优（排序），点精度口径作废 |
| 方案优先级 | B 当前重点，A 已满足 | **B（wave 全覆盖）最高优先，A（LIB）降级** | LIB 数据已就位、代码待加速；wave 全覆盖对降预测噪声的杠杆最高 |
