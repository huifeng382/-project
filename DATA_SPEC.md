...
```
data/batch1/circuit_static.parquet   # 第一批电路静态数据
data/batch1/timing_arcs.parquet      # 第一批仿真动态数据
data/batch2/circuit_static.parquet   # 第二批电路静态数据
data/batch2/timing_arcs.parquet      # 第二批仿真动态数据
```
...
| expr编号范围 | `expr0200` ~ `expr0549`，不得与第一批重叠 |
| 筛选要求 | 按结构特征去重（门类型组成+深度+扇出分布），避免拓扑高度相似的电路重复出现 |
| 引脚要求 | 全部为4引脚（a,b,c,d） |
| 每电路corner数 | 9（3 slew × 3 load 全交叉） |
| 每corner组合 | 4个引脚 × 2个方向 × 2个vector = 16行 |
| 每电路总行数 | 9 × 16 = 144行 |
| 第二批总行数 | 约43,200-50,400行 |

### 两批合计：约 100,800-122,400 行 ≈ 10万样本
...
1. 生成第一批数据，运行模型训练，确认slew/load依赖关系已被学到
2. 若第一批误差不达标，增加第一批电路数（而非调整第二批）
3. 第一批达标后再生成第二批，验证结构泛化能力
