...
# 并列判定：位级相等（17.2.4 撤销 17.2.3 的相对容差，回到历史口径）。
# 2026-09-15 定因：进程间抖动的源头**不在并列判定**，而在边序构造 ——
# src/graph_builder.py parse_netlist 原为 list(set(edges))，字符串元组集合的迭代序随
# PYTHONHASHSEED 变 → edge_index 行序变 → 前向的 float32 累加序变。抖动量级是
# **float32 的 1 ulp（~1e-7 相对）**，而 17.2.3 的容差取 1e-9 —— 小两个量级，该合的合不上，
# 实测无效（同一 ckpt 两个新 serve 进程仍差 478~814 行）。源头已在 17.2.4 定序，
# 故这里不再需要任何容差：`==` 既不依赖进程身份，又保持了历史 12 点表的口径。
def predict_rank_batch(models, cands, scaler, device):
    ...
    17.2.4: 并列判定回到位级相等（根因已在 graph_builder 的边序构造上定序，见上）。"""
    rows = []
    for c in cands:
