...
> ⚠ **本表「Rust 严格@3 / Rust 选择遗憾 / Rust 两阶段」三列全部是 106 集 window 池化口径**（历史）。2026-09-16 定案：**部署真值 = 714 集 batch 口径，比本表大一截且增幅因臂而异**（v2iaa42m4 严格@3 30.2%→56.7%、v2nowave42m4 34.9%→74.4%）→ **本表三列只作臂间同口径比较，绝对值不可外引**。详见下方「部署口径定案」块与 `docs/GNN_RUST_DATA_DIFF.md` §15。
...
**(2) 17.3.11 四口径对照读数**（v2nowave42m4，serve 8000，`midpoint_ep250.pt sha1=09be9a3c53b90760`，46 个 CSV 快照 09-15 21:03–21:05；**714 集 / 5,390 成功 / 8 失败**，与 §13.2–13.6 各 run 逐项相同）：

| 口径 | 单位数 | 严格 k2 | 严格 k3 | 宽松 k2 | 宽松 k3 | 选择遗憾 | 两阶段 |
|---|---|---|---|---|---|---|---|
| **① 全部批（部署真值，默认）** | **714** | **53.1%** | **74.4%** | **69.6%** | **97.6%** | **7.23%**（中位 3.88%） | **1.66%** |
...
其余 ①：cap2 95.55%、Spearman 0.546、hi_spread 591 集、小集（<4 候选）167。③ 只有 18 个单位（46 个 CSV 里 28 个无合格集）。④ 可反解 DEPTH_MIX 自身：严格 k3 **77.4%**、遗憾 **7.92%** ⇒ **它的毛病在「权重」不在「质量」**（538/714 = 75% 权重压在 1/18 个电路上）。
...
**(5) ⚠ 口径效应不是常数 ⇒ 旧口径的跨臂比较不可外推。** 同一 arm 同一 CSV，只换分组键：iaa 严格@3 30.2% → **56.7%（+26.5pp）**；nowave 34.9% → **74.4%（+39.5pp）**；而遗憾方向相反（iaa **+0.7pp**、nowave **−5.8pp**）→ **失真量 model-dependent，不是可加的固定偏置**。⇒ I7 的六臂结论都产自同一 106 集口径、**臂内可比性仍在**，但**与部署真值的落差因臂而异、不可外推**；本页上方所有「Rust 选择遗憾 10.87/11.13/12.19…」一律按 **106 集 window 口径**解读。**判别规则**：recall 升而遗憾不降 ⇒ 口径产物；两者同向 ⇒ 真实能力（iaa 换口径 +26.5pp/遗憾 +0.7pp 反向 ⇒ 产物；nowave 相对 iaa +17.7pp/遗憾 −8.7pp ⇒ 真实）。

**(6) 判据稳健性不同**：**严格 recall@3 ≥90% 四种口径全不达标**（74.4/68.9/79.5/65.3；宽松 97.6/95.0/96.9/90.3）→「GNN 只做启发式粗排」**与口径之争无关，成立**。**选择遗憾 ≤5% 只在 ② 过**（4.76%），① 7.23 / ③ 5.67 / ④ 5.11 全在线上 ±0.7pp 内 → 诚实读法 = **「≈5%，临界」，不是达标**。
...
**(8) 交付基线读数刷新（本次「最优结果」更新）**：

| 交付基线 | ckpt | 口径 | 严格 k3 | 宽松 k3 | 选择遗憾 | 两阶段 |
|---|---|---|---|---|---|---|
| **v2nowave42m4（纯拓扑）** | `midpoint_ep250.pt`（sha1 09be9a3c53b90760） | **714 集 batch（部署真值）** | **74.4%** | **97.6%** | **7.23%**（中位 3.88%） | **1.66%** |
| 同上（历史记录，同 ckpt 同 arm） | 标称 ep250 / mid200（见 (7)） | 106 集 window（历史池化） | 39.6% / 34.9% | 65.1% / 66.0% | 10.87% / 11.13% | 3.95% / 4.00% |

**⚠ 上下两行不是同一把尺，禁止横向对比、禁止互相印证。** 结论：① **交付基线不变**（纯拓扑 nowave 路线，serve 现钉 `midpoint_ep250.pt`）；② **主判据仍未达标**（严格 74.4% < 90%、遗憾 7.23% > 5%），缺口要动模型/数据（60w 新战役）—— 与 16.11.39 裁决同向，只是量级换了尺子；③ **不改 serve、不改交付基线**（承用户 2026-09-15 决定）。
