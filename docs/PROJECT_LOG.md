...
**⚠ 随数一起记的三条边界。** ① ~~两臂的特征表不同~~ → **两臂 `in_features` 探针实测都是 45**（2026-09-22：`convs.0.lin_rel.weight` 第 1 维；基线 `midpoint_ep250.pt` 与 V3 `midpoint_ep200.pt` 各测一次）⇒ **输入布局同一、都「不带 env」** ⇒ 本趟比较**不掺特征表变更**，差异只在**数据**（`v3_delivery` vs `batch_v2_*`）与 ckpt。⚠ **本条初稿写错，见 §18.5.2**：曾按 I9 旧记写成「基线 in_dim 14 / V3 45（= 14 静态 + 31 弧特征）」——**两处都错**（数字错，实测皆 45；且把日志 `1 static + 31 arcs` 的「31」误当特征数，**那是 31 个 `timing_arcs_partNN` 分片文件**，18.0.0 原记即如此）。② **106 集 window 口径（已废，仅存档）**：V3 严格k3 44.3 / 宽松k3 66.0 / 遗憾 6.99 vs **同口径重算**基线 47.2 / 70.8 / 9.59 —— 且 V3 该口径下 **12/106 集量纲混合守卫 ❌**（集内「秩」与「原始延迟」混排 ⇒ 排序无意义）⇒ **不得作判读依据**；另：§6.7 存的 106 集基线行（遗憾 11.13 / 严格k3 34.9）与今天同树重算（9.59 / 47.2）对不上，**已记 I21 待查**（batch 口径逐位复现 ⇒ 不是分析器漂移）。③ **排除掉的三个机制**：**不是并列**（同版分析器下「预测有重复值」57 vs 56 集、「第3==第4」5 vs 8，解释不了 15pp）、**不是候选集或特征码错配**（候选集逐项相同；serve 侧 8 个文件 blob 逐行相同）、**不是错标树**（argv + ckpt sha1 + 分析器戳三处一致）。

**18.5.2 修正（同日，无代码改动）。** 恢复 serve 前的 §6.2 Step 1 探针实测交付基线 `~/project-107-v2nowave42m4/outputs/midpoint_ep250.pt`（sha1 `09be9a3c53b90760` 复核一致）的 **`in_features = 45`**，**不是 I9 旧记的 14** —— 与「§6.2 决策表：45 ⇒ 不带 env」及「交付基线历来不带 env」三方一致 ⇒ **旧记 14 作废，I9 已改**。连带作废本节由它派生的那条边界（「两臂特征表不同 ⇒ 归因分不开」）：**两臂 `in_features` 都是 45** ⇒ 本趟差异**不掺特征表变更**，−15pp **可归因到「V3 这一版训练出的模型」**；仍未排除的只有**单 ckpt 采样**（ep200 vs ep250 不同 epoch）。该 14 的出处未查（疑为 run_stamp 里另一个字段），已记 I15 同类第 3 例。**换 serve 是本次探针的顺带产出：交付基线维持 ep250 ⇒ serve 恢复目标不变。**
...
### ✅ 17.5.0/17.6.0/17.6.1 / 18.7.0/18.7.1/18.7.3 — 全组仿真最好者胜 ＋ 预算闸门与两道保险（2026-09-25）
...
**⑥ 服务器装包（2026-09-25 已跑，全绿）**：重传 ⇒ `sha1sum -c` 清单 **`OK_lines=56`**（传输包 `b54c5ea4dce98cb02a64da14f3e7142f7db2f8fb`、清单 `1f7d89fac6dcbc5a41f5d00632d6d595ed07c2fe` ⇒ **`RUST_FP = 1f7d89fac6dc`**）⇒ `cargo build --release --tests` **`build_rc=0`**（上一轮 `rc=101` 的 `format!` 占位符事故已修，见 18.7.2 行）；「代价实测」趟按你的 2026-09-25 指示**跳过**。

**⑦ 正面验证四趟（2026-09-25，逐趟实测）**：

| 趟 | 验什么 | 实测读数 |
|---|---|---|
| `n` 正常路径 | 不设旋钮时零变化 | `rc=0`／157 s／`accepted=21 iters=40 evals=5539 failed=0 backtracks=19 max_depth=21`／**无** `stopped=`／`NA_rows=0`／分析器 **638 窗全 0** |
| `w` 电路墙钟 | `TL_MAX_WALL_S=120` 咬合 | `rc=0`／120.0 s／`iters=28 evals=3797 accepted=15 backtracks=12`／**`stopped=wall`**／分析器 **436 窗全 0** ⇒ **末组仍完整**（`pos` 全集不变式没被切碎） |
| `t` 单次超时 | `XYCE_BIN=~/slowsim.sh` 拖慢 + `XYCE_TIMEOUT_S=1` | `rc=0`／`FAIL …: simulation timeout: cmd = …`（1.12 s）／NA 行 1／批次**继续** |
| `x` 连续超时中止 | 同 `t` 跑到组尾 | `rc=0`／**修前** `连续 0 次仿真超时（XYCE_TIMEOUT_S）⇒ …（已跑 9 步、backtracks=2）` = **行为对、数字错** ⇒ **内层 17.6.1** 修后复跑 `连续 20 次…⇒ 中止该电路（已跑 11 步、backtracks=3）` |

**⚠ 三个坑（已写进 OPERATIONS §6.9 第 2 步）**：① 模板的超时守卫**只认正整数**（`[ "$V" -gt 0 ]`，`0.001` 等于「没设」）⇒ **1 秒是最小可设值**；② **1 秒 > 一次真仿真**（实测 level0 ≈0.045 s、level4 ≈0.28 s）⇒ `XYCE_TIMEOUT_S=1` **根本测不出超时**，第一版验证脚本据此白跑一趟（那趟反而成了有效的**反面对照**：大批真实评估、零误杀）；要正面触发**必须把仿真拖慢**，用 `XYCE_BIN` 包装器（模板里唯一引用点 ⇒ **零代码改动**的合法手段）；③ **分析器要在 venv 里跑**（裸 `python3` 无 numpy ⇒ `rc=1` + 零读数，看起来像「树是空的」，极易误判）。

**⚠ 一次单位口径订正**：`XYCE_TIMEOUT_S` 限的是**一次 Xyce 调用**，不是一整个候选（一个候选 = Σ_口(可翻转该口的输入脚数×2) 次调用）。实测 level4/ADD4_OVF 单次 **≈0.28 s**（10 秒直接探针复核 0.303 s）、level0/AND2 单次 **≈0.045 s** ⇒ `1800 ÷ 0.28 ≈` **6400 倍**，**不是** DIFF §23.7 原先写的「≈最慢单候选的 64 倍」（那是拿**候选**耗时当分母）—— 已在 §23.7/§23.9 订正。⇒ 该机制在健康机器上**实际不可达**，它挡的是「仿真器真卡死」那类形态；全量的墙钟压力**全在 `TL_MAX_WALL_S`**（§23.7 的定档算术不受影响）。

**⑧ 两个环境闭环证据**：① **10 秒直接探针**（直接跑渲染后的 `cmd_expr_…_fall.sh`）：默认 `rc=0`（0.303 s）／`XYCE_TIMEOUT_S=1` + 慢包装器 ⇒ `rc=42`（1.002 s，stderr 有 `XYCE_TIMEOUT: killed after 1s`）／慢包装器但**不设**超时 ⇒ `rc=0`（5.003 s）⇒ env 真传到了渲染脚本、退出码 42 契约成立、**默认分支逐字不变**。② **`XYCE_CACHE_DIR` 覆盖真生效**：新目录落到 **11,858** 个文件，默认 `temp_sim_cache/` 的 mtime **未被触碰**（仍是 09-23 00:08）⇒ 全量写新目录、旧缓存安全。

**⑨ 全量启动（2026-09-25 19:54:32）**：`run_shadow_batch.sh bestfirst` ⇒ `Rust 源指纹 1f7d89fac6dc`（与传输包清单前 12 位逐位相同 ⇒ **跑的就是验收过的那棵树**）、`模式 env` 带全三旋钮 + `XYCE_CACHE_DIR=…/temp_sim_cache_bf20260923`、serve ckpt `midpoint_ep250.pt` sha1 `09be9a3c53b90760`、12 分片起、收尾挂到 `~/shadow_analyze_bestfirst.out`。**待回填**：R1/R2（严格 + 宽松、四行分层）、NA/超时计数、fail-loud 计数、墙钟实际触发次数；随后第二臂换 ckpt 复跑。
