...
| v2nowaver42m4 🔜 | 纯拓扑 × rank 直训（RANK_LOSS_W=0.5, 17.0.4 空格2） | 运行中 | — | — | — | — | — | — |
| v2iagr42m4 🔜 | GBDT15 × rank 直训（RANK_LOSS_W=0.5, 17.0.4 空格1） | 运行中 | — | — | — | — | — | — |
...
- **17.0.5 A/B 首轮（2026-09-04）= 无效对照，勿引用数值**：env 旋钮（K/HID/EMB/LR/PATIENCE/CONE_FEAT）入服务器脚本后，A（K5/H160/E32·LR3e-3·PAT40）/ B（+锥体）双双 best_val 0.3571/0.4257 且落 ep1/5、early-stop@ep45，train_loss 冻死 = **LR3e-3 × 大模型 4-5× × cosine220 前段不退火 × patience 早掐** 的配置错配，**非容量方向被判死、非代码回归**。唯一正信号 = B 坏训练下 m4 桶 0.7579 > 基线 0.6583（锥体对 m4 有真增益，待干净复验）。**修正 A2/B2（LR1e-3·PAT0·EPOCHS80·不开 N_CAP）已再跑**，判定以后续为准。详档 §4.3 见 `docs/IDS_AVG_GNN.md`。
