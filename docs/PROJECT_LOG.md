...
**⚠ 三行是同一 arm 同标称 ckpt 的三把尺，禁止横向对比。** 第 1 行是本次新数（部署口径），第 2、3 行是历史记录且**彼此也不一致**（严格@3 差 4.7pp，见 (7)）→ 只作存档。结论：① **交付基线不变**（纯拓扑 nowave 路线，serve 现钉 `midpoint_ep250.pt`）；② **主判据仍未达标**（严格 74.4% < 90%、遗憾 7.23% > 5%），缺口要动模型/数据（60w 新战役）—— 与 16.11.39 裁决同向，只是量级换了尺子；③ **不改 serve、不改交付基线**（承用户 2026-09-15 决定）。
