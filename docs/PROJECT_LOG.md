...
**(3) ⚠ 17.4.1 的两个附带影响。**
- **全库 `train_sweep.py:NNN` 引用整体 +55**：(1) 在 `src/train_sweep.py:25` 插了 55 行 ⇒ 第 24 行以下所有钉死行号平移。已改**政策操作性引用**为**按锚点找**（`OPERATIONS:212` 与本节 `:1100/:1103` —— `grep -n 'cap2={score' src/train_sweep.py`，17.4.1 时 `:1128`）。**未改**（历史分析记录，改行号需连同结论复核）：`DIFF:952/960`、`PROJECT_LOG:921/956`、`GNN_CODING_LESSONS:19`。
- 🔴 **比行号更重的发现（未擅改）**：`train_sweep.py:1113` 注释写明「**17.3.7 换判据：原 score（`100·r3+50·r2+0.3·sp−0.2·regret+0.1·cap`，recall@3 主导）**」⇒ **Best midpoint 的加权 `score` 判据自 17.3.7 起已不存在**（现为 `mrk.get('capture2_pct')`）。因此 `DIFF:952` 与 `PROJECT_LOG:956`（那条「⚠ 纠一处判据名错误」）**描述的是已删掉的代码**，且 `BEST_METRIC` 这个名字也已改名 `BEST_MODEL_METRIC`。核心结论（该 selector 序与部署序反序）**未必失效，但它挂在哪个判据上需重述**，且与 `BEST_MODEL_METRIC='capture2'` 的 ❌ 判决是同一件事的两面 ⇒ **与 D1 定档一并处理**。
