...
| **规模覆盖** | 晶体管上限 184（>184 = 0%） | **≥10% 电路 180~400+ 管**（对齐 Rust ENC8/ALU2/ADD4_OVF 189-316 管） |
| **深度覆盖** | 深度全 ≤5（>6 = 0%） | **≥10% 电路深度 >6**（含深链 8-15 层，优先「窄而深」15-25 门串 8-15 层） |
...
| **I/O 形状** | full(4入1出) + rest(多形状) + m4(5 多输出形状) 拼接 | **单一数据内含全形状**（1~16 入 / 1~6 出，单输出为主 + 多输出 ≥20%） |
...
| **组织形式（V3）** | **单一数据集**（一个 static + 一个 timing_arcs，~5 万电路/~60 万行；parquet 可拆多文件但同属一集） |
...
>
> **新增约束**（在 I/O 分桶**之上**叠加）：
> 1. **晶体管规模配额**：总量中 ≥10% 落在 **180~400+ 管**区间（对齐 Rust ENC8/ALU2/ADD4_OVF 级 189-316）；>250 管至少几百个电路。上一轮所有 batch >184 管为 0%。
> 2. **深度配额**：≥10% 的电路**深度 >6**（含 8-15 的深链），上一轮 >6 深度为 0%。深度 = 最长门链（从输入到输出的最大串联门数）。**优先「窄而深」（15-25 门串 8-15 层）而非只加门数**。
