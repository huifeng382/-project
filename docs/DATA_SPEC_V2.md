...
> ⚠ **待复核（2026-09-03）**：Rust `build_simu_vectors_for_simulation`（design.rs）按 **per-(output, pin)** 各自 break（外层 out 循环，不同 output 的 vector/truth_table_idx 可不同）；本规格「每 (pin, dir) 固定 1 个 vector」与 Rust 语义是否一致，需对照 Rust 仿真配置与训练数据实际 vector 数验证后再定稿。
...
> - **Tier B · Rust 邻域多样化（~63% 行）**：不锚定具体档位，但**不脱离 Rust 使用域**——形状 ∈ Rust 可达范围（1~16 入 / 1~6 出）、trans ≤ ~400（316-400 少量，泛化余量）、深度 ≤ ~18；优先补 Tier A 档位**邻近过渡带**密度（140-190 管、深 6-8 等）；禁止 Rust 域外形态（超深无意义链、超宽纯拼接、奇异多输出如 (16,3) 式）。
...
> 5. **Tier A/B 行数占比（~37/63）统计**；Rust 域外形态（trans > 400、深度 > 18、奇异多输出）行数占比 <5%；
> 6. **泛化闸门（硬，2026-09-03 执行口径修正）**：新数据按 expr 切 train/val/test；另以**旧数据固定 test 集**（v2wave42m4 同一切分；新数据 expr 与旧数据不重叠，故此为跨分布评估）作为通用 test——新模型在该旧 test 集上的组内排序指标（选择遗憾 / Spearman / top1）**不得低于 v2wave42m4 同口径基线**；低于 → 过拟合信号，需降 Tier A 占比 / 增功能多样性后重生成；
