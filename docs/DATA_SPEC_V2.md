...
> 5. **含近延迟候选对**：组内须有延迟差 <10% 的近延迟变体（上一轮组内 spread 中位 157.8%，全大差；Rust 有 9.1% 集 <5%）。
...
> 5. **近延迟候选对**：同一 expr 组内除结构差异大的变体外，**须含延迟差 <10% 的近延迟对**（上一轮组内 spread 中位 157.8%，全是 >10% 的大差；Rust 候选集 9.1% 是 <5% 的近差，模型没学过细粒度分辨，recall@1 仅 11%）。
>
> **量化验收**：交付 metadata.json 必须含 **transistor_count 分位数（p50/p90/max）与深度分位数**，供接收方核对上述配额达标。
