...
> **🔴 V3（新 60 万行）相对上一次生成（V2 full/rest/m4）的修改点清单（2026-09-02 实测后）**：
> 1. **新增规模配额**：≥10% 电路晶体管 180~400+ 管（上一轮 >184 管 = 0%）。理由：Rust ENC8/ALU2/ADD4_OVF 候选 189-316 管，训练从未见过。
> 2. **新增深度配额**：≥10% 电路深度 >6（含深链 8-15 层；上一轮 >6 = 0%）。理由：Rust 深电路（表达式深度 8-12）recall@3 仅 12-20%。
> 3. **显式展开、禁巨复合门**：上一轮 m4 用 SC_JOIN_* 巨复合门压缩顶层 X_ 数（门数统计失真）——V3 用基本门显式展开。
> 4. **含弱驱动中间门**：深链/大电路含驱动差异大的中间节点（serve 对 Rust 候选 6.1% 负值门未覆盖）。
> 5. **变体延迟分布量化（16.11.20 修正——原「补近延迟对」不成立）**：实测训练数据组内**相邻变体延迟差 med 0.1%、93% <5%**（full/rest），Rust 候选集 med 4.4%、55% <5%——**训练已有（甚至更密的）近延迟对，不缺**。真正的分辨率问题在模型（huber 绝对延迟 loss 未学排序），不在数据。→ **本条改为要求**：组内变体延迟差**覆盖多档**（含 <1%、1-5%、5-20%、20%+ 各档），避免全挤在 <1% 或全大差——量化验收见 metadata（组内相邻差分布）。
> 6. **metadata 量化验收**：交付须含 transistor_count 与深度的分位数统计。
>
> 详细约束见「四、批次要求」的**规模/深度多样性要求**。其余字段/格式/质量规则与 V2 相同（V3 只改多样性配额，不改列结构）。
