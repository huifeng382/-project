...
| **规模覆盖** | 晶体管上限 184（>184 = 0%） | **档位锚定（2026-09-03 修正）**：189-233 与 233-316 两档按形状配额（Tier A ≈37% 行）；Rust 邻域 ≤400（Tier B ≈63% 行）；全局「≥10% 180~400+」仅统计参考 |
...
| **门表达** | 巨型复合门 SC_JOIN_* 把整电路压成 ~15 个顶层 X_（m4：trans 68/顶层 15 门 vs Rust 候选 X_ med 32/p90 57/max 73） | **SC_JOIN_* 宏层次 + 宏粒度对齐 Rust（2026-09-03 修正，宏层实测校准）**：宏 = Rust `expr_to_hierarchical_spice` 同套（名称编码结构 SC_JOIN_*；宏 trans 分布 med 5、≤14 为主、偶有 16-84 管多输入复合，非「每宏 2-8 逻辑」）；X_ 数与 Rust 候选同量级（med 32、大电路 40-73）；禁止 m4 式 ~15 个超大宏打包（宏粒度是深度/规模验收可比的前提） |
| **中间门驱动** | serve 对 Rust 候选 6.1% 负近似门，训练 0%（弱驱动中间门未覆盖） | **深链/大电路含驱动差异大的中间节点** |
| **组内延迟差** | 相邻变体差 med 0.03%、<1% 占 82%（train/val/test 一致）——全挤超近差 | **组内变体延迟差多档覆盖（对齐 Rust 实测）**：≥80% 组含 ≥2 个差 >10% 变体 + 四档占比对齐 Rust 20/35/39/6 ±10pp + spread 中位 30-60%（详见锚定小节第 5 条） |
...
> 详细约束见「四、批次要求」：**形状-规模-深度锚定（### 锚定小节，最核心）** + 规模/深度多样性要求（历史背景）。注：因单一数据集，**不再有「默认 batch 组合」问题**——训练直接用整个数据集按 expr 切分。
>
> **训练数据角色（2026-09-03 写死）**：**V3 单一数据集为唯一训练数据**；旧 full/rest/m4 **不再入训练**，仅旧数据固定 test 集用作泛化闸门（见锚定小节验收 B 段）的跨分布评估。
...
```
data/circuit_static.parquet    # 全部电路（~3-5 万电路——电路数由 60 万行预算反推，见第四节备注；单一文件或按 parquet 分区拆多文件均可，但属同一逻辑数据集）
data/timing_arcs.parquet       # 全部仿真行（~60 万行）
data/metadata.json             # 验收统计：shape_coverage（逐形状 trans/深度分位 + Rust 参考档）+ Tier A/B 占比 + 组内相邻差分布 + 弱驱动中间门占比
data/sc_expansion.json         # 【必须，2026-09-03 补列】覆盖 V3 全部 SC_ cell 的宏展开表（宏粒度对齐 Rust 后 cell 集与旧 3868 宏表不同，需新交付；见 §五规则 15/16）
```
...
| `transistor_count` | int | `44` | 电路中的晶体管总数（**= 全部 `.SUBCKT`（含 SC_JOIN_* 宏内部）的 `M_` 实例总数**，2026-09-03 明确口径；禁止只数顶层） |
...
| 项目 | 值 | 来源 |
|---|---|---|
| slew | **2 ps** | Rust `VSTIM_RISE ... PWL(0 0 20p 0 22p {VDD} 1n {VDD})`（翻转沿在 **20-22ps**，2ps slew） |
| load | **1 fF** | Rust `Cload_out __PIN_OUT__ 0 1f` |
| corner 标签 | **`s02p0_l01p0`** | — |
| 翻转输入 | **仅 switching pin 翻转**（接 VSTIM_RISE/FALL）；**其余输入按 vector 固定电平（vdd/gnd 常量），不翻转** | Rust `tb_*.sp` 的 `XUUT` 实例化（switching pin = stim_rise/stim_fall，其余 = vdd/gnd） |
| 仿真窗口 | `.tran 1p 1n`（1ps 步长 / 1ns 窗口） | Rust `asap7.sp` |
| 输出负载 | **每 (output) 单独 tb 文件，仅被测输出接 1fF** | Rust `tb_*_vec_*_rise/fall.sp`（out_00..out_M 目录各一 tb） |
...
要求（2026-09-03 修正——原「每 (pin, dir) 固定 1 个 vector」与 Rust 语义不符，已实测确认）：
- **每个 (output, pin) 组合各取一个 break vector**（对齐 Rust `build_simu_vectors_for_simulation`：对每个输出、每个输入引脚，取真值表中**第一个让该输出翻转**的行 truth_table_idx，该 pin 位 = 0）。不同 output 的同一 (pin) 的 vector 可以不同。
- **rise / fall = 同一 break vector 的双向激励**（对齐 Rust `simulation.rs`：rise 用 truth_table_idx 行，fall 用 `truth_table_idx | (1 << pin_index)` 行——实测 m4 数据 rise/fall vector 关系与此完全一致，11880/11880 组验证）。
- 非切换引脚的电平由该 truth_table_idx 决定（Rust 取「第一个能翻转输出的真值表行」，非任意选）。
- 行结构：每 (output, pin) 2 行（rise + fall）→ 每电路 2 × N_in × M 行（与第四节行数公式一致）；**同 (pin, direction) 下不同 output 行的 vector 各异**，不是共享 1 个 vector。
...
| I/O 形状 | 任意 N-in（1~16）/ M-out（1~6）；**Rust 18 实测形状全覆盖 + 每形状规模/深度档配额**（详见下方「### 形状-规模-深度锚定」小节） |
| corner 数 | **1**（2ps slew / 1fF load，标签 `s02p0_l01p0`） |
| 每输入引脚 | 2 个方向（rise / fall） |
| vector 数 | **每 (output, pin) 1 个** break vector（共 ≤ N_in×M 个不同 vector；rise/fall = 同 vector 双向激励） |
| 每电路行数 | **≤ 2 × N_in × M 行**（仅生成「该输出对该输入敏感（能翻转）」的 (output, pin) 对——对齐 Rust `break` 语义；预算按全量 2×N_in×M 上限核算） |
...
> **⚠ 锚定对估算的影响（V3，2026-09-03 修正）**：下方「### 形状-规模-深度锚定」Tier A（18 形状合计 ~7900 电路 ≈22.2 万行 ≈**37% 行**；★ 多输出大形状行成本高——如 (9,6) 每电路 ≤108 行、ADD4_OVF 类按敏感对实为全量 78%（Rust 实测））+ Tier B（Rust 邻域 ≈63% 行）。多输出占比显著高于旧数据 → 平均每电路行数 > 12，**实际电路数将低于 5 万**。行数预算以 **60 万行封顶**（行数按敏感对实算可少于上限），电路数按实际行数反推；Tier A/B 占比按行数核算。
...
>
> （2026-09-03 衔接说明）分桶约束作用于 **Tier B 与整体统计**（保证 18 锚形状之外的小输入电路仍有覆盖）；**Tier A 按「### 形状-规模-深度锚定」表执行，不受分桶约束**。
>
> **🔴 输出侧约束已修订（2026-09-03）**：旧版「输出 1/2/3+ 三档、多输出 ≥20%」是**比例**约束——实测证明它防不住「形状对、规模缩水」（上轮 m4 教训，见下方锚定小节）。**V3 输出侧改用下方「### 形状-规模-深度锚定」**：Rust 18 实测形状全覆盖 + 每形状电路数配额 + ★ 规模/深度档 + ◆ 次档（结构模式锚定：功能任选 + 链式结构 + 实测达标，2026-09-03 二版）。单输出仍为主（Rust 46 电路单输出占 35/46），但不再以「占比」作为验收口径。
...
> ① **m4「形状对、规模缩水」**（2026-09-02）：5 种 Rust 多输出形状 (4,3)(5,5)(7,4)(8,4)(9,6) 形状名全在，但形状内 trans 全缩水——(7,4) 58-66 vs Rust ALU2 235-262（~4×）、(8,4) 66-74 vs ENC8 189-233（~3×）、(9,6) 110-118 vs ADD4_OVF 247-316（~2.2×）、(4,3) 26-30 vs ENC4 56-69；仅 (5,5) 达标。网表证据：m4 用 SC_JOIN 拼形状（trans 68/顶层仅 15 门 vs Rust 候选 X_ med 32、大电路 40-73 个）。
...
> - **Tier A ~37%（行，2026-09-03 按表内配额复算；下表 18 形状配额即此）**：形状 × 规模 × 深度三元组按 Rust **候选实测**锚定；
> - **Tier B ~63%（行，60 万 − Tier A）**：**Rust 邻域多样化**——不锚具体档位，但不出 Rust 使用域（见下），替代旧数据的完全随机枚举（预算浪费在 Rust 用不到的远端结构）。
...
| 浅（归因定案） | DEPTH_MIX（5,1） | 4/6/6（0% >6） | 39-117 | **非深度 OOD（11.28 定案）**：候选 X_ 链 0% >6；538 组遗憾 med 7.65% vs 全体 4.89%（高 2.8pp，p75/p90 与全体重合）；spread 51% ≈ 全体 48%、训练 (5,1) spread 361% 覆盖充足 → 旧「纯深度 OOD 代表」撤销；按 spread 大变体生成，不定深度档 |
...
> - **Tier A · Rust 档位锚定（~37% 行；18 形状配额即此，详见锚定清单表）**：形状 × 规模档 × 深度档三元组全达标。
> - **Tier B · Rust 邻域多样化（~63% 行）**：不锚定具体档位，但**不脱离 Rust 使用域**——形状 ∈ Rust 可达范围（1~16 入 / 1~6 出）、trans ≤ 316 为主（Rust 实测全体 max=316；316-400 扩展 ≤10%）、深度 ≤ ~18；**分布判据（2026-09-03，防退回旧 rest 浅小铺量）**：Tier B 内 trans p50 ≥ 120 且 140-190 管过渡带 ≥20% 行、深度 6-8 过渡带 ≥15% 行（衔接 Tier A 档位，保证邻近密度）；禁止 Rust 域外形态（超深无意义链、超宽纯拼接、奇异多输出如 (16,3) 式）。
...
> **★ 档弹性条款（2026-09-03）**：★/◆ 档样本需 (trans, 深度) 双下界过滤且功能空间窄（(9,6) 达深 ≥9 + trans ≥247 的功能基本是加法/比较类），枚举合格率低属预期；**若某 ★/◆ 档过滤后产不足，优先保 ★/◆ 档配额，可削减普通/多输出形状配额补足（60 万行封顶不变）**。
>
> Tier A 合计 **~7900 电路**（表内 18 形状配额之和，≈22.2 万行 ≈37% 行）；多输出大形状行成本高（每电路 2×N_in×M 行，如 (9,6) 108 行），按行预算平衡，**总量 60 万行封顶**；Tier A/B 占比按行数核算。
...
> - 锚定电路只占 Tier A（~37% 行），其余 Tier B 邻域多样化 + 每组 10-15 变体功能等价——模型不会只见 Rust 那几族功能。

**量化验收（2026-09-03 拆分）**——**A. 交付验收（生成方责任，metadata.json `shape_coverage` 扩展）**：逐形状记录 `{n_circuits, trans_p50, trans_p90, trans_max, depth_p50, depth_p90}` + Rust 参考档（代表电路 / trans med / trans max / 深度档），规则：
>
...
> 5. **Tier A/B 行数占比（~37/63）统计**；Rust 域外形态（trans > 400、深度 > 18、奇异多输出）行数占比 <5%；Tier B 分布判据：`trans_p50 ≥ 120`、140-190 管过渡带 ≥20% 行、深 6-8 过渡带 ≥15% 行；
> **B. 接收方训练后闸门（非生成方交付项，2026-09-03 归属修正）**：新数据按 expr 切 train/val/test 训练后，以**旧数据固定 test 集**（v2wave42m4 同一切分；新数据 expr 与旧数据不重叠，故此为跨分布评估）作为通用 test——新模型在该旧 test 集上的组内排序指标（选择遗憾 / Spearman / top1）**不得低于 v2wave42m4 同口径基线**；低于 → 过拟合信号，需降 Tier A 占比 / 增功能多样性后重生成；
...
> 3. **宏粒度对齐（2026-09-03 修正——原「用基本门显式展开」表述与 Rust 实测相反；11.28 宏层实测校准）**：Rust 候选 dut .sp 实测为 SC_JOIN_* **宏层次**（非基本门展开）。V3 沿用宏层次（同 `expr_to_hierarchical_spice`），宏与 Rust 同套（名称编码结构）：Rust 实测 **1064 种宏**、宏 trans med 5 / p90 14 / max 84（2-6 管小宏为主 + 8-84 管多输入复合宏）、X_ 数 med 32 / p90 57 / max 73。**约束**：宏命名/结构沿用 Rust 生成器；X_ 数落在 Rust 量级（20-73，中位 ~32）；禁止 m4 式把整电路压成 ~15 个超大宏。宏粒度不一致会使「X_ 宏级深度 / trans」验收失真。
> 4. **弱驱动中间门**：深链/大电路中含**驱动强度差异大的中间节点**（小扇出/弱驱动/高寄生），覆盖 Rust 候选的这类门（serve 对 Rust 候选算近似特征出现 6.1% 负值门，训练数据 0%——结构未覆盖）。**验收判据（2026-09-03）**：★ 深/大档形状内，含「弱驱动中间门」（实例扇出 ≤2，或对地寄生 ≥ 该电路寄生中位 2×）的电路占比 ≥30%；metadata 报告中间节点扇出/寄生分布。
> 5. **变体延迟差多档覆盖（16.11.20 修正——原「补近延迟对」不成立；2026-09-03 验收口径对齐 Rust 实测）**：实测训练组内相邻变体差 med 0.1%（93% <5%），**不缺近延迟对**。真正问题是模型未学会排序（训练目标），不在数据。为防组内变体**全挤在 <1%**（无排序信号梯度），要求组内延迟差**多档覆盖**。**验收线（Rust 714 有效候选集实测 2026-09-03）**：① **≥80% 的 expr 组含 ≥2 个与组内最优差 >10% 的「拉开距离」变体**（对齐 Rust 实测 81% 组自然满足；不强求每组 100%——Rust 19% 近差功能组天然达不到，排序难度低，强制会逼生成方硬造不真实变体）；② 组内相邻差四档（<1% / 1~5% / 5~20% / >20%）全局占比对齐 Rust 实测（20/35/39/6%）**±10pp**：<1% 10-30% / 1-5% 25-45% / 5-20% 30-50% / >20% 3-10%——防旧数据「近差为主 + 少数极慢 outlier」形态（训练相邻差 med 0.1% 的病）；③ 组内 spread（延迟跨度/中位）参考中位 **~30-60%**（对齐 Rust 实测 med 48%、p75 58%、p90 72%；旧训练 (5,1) spread med 361% 属 outlier 拉高形态，避免）。
>
> **量化验收**：交付 metadata.json 必须含 **① transistor_count 分位数（p50/p90/max）；② 深度分位数；③ 组内变体延迟相邻差分布（<1% / 1-5% / 5-20% / >20% 各档占比，按 Rust 参考 ±10pp 核对，见上第 5 条）+ 组内 spread 中位；④ 弱驱动中间门占比（判据见上第 4 条）**；并附 `shape_coverage` 与 Tier A/B 占比（见「### 形状-规模-深度锚定」验收），供接收方核对上述配额达标。
...
> **早期模型排序指标（2026-08 前诊断背景，已过期）**：当时 Spearman ≈ 0.21、选择遗憾 ≈ 2.6%、top1 ≈ 42%；当前 v2wave42m4（2026-09）已到 Spearman 0.688 / 遗憾 0.65%（见 PROJECT_LOG 16.x）——本节方案 B/C/D 为当时降噪杠杆，wave 方向已被 v2wave42m4 验证。
...
> ⚠ **宏语义说明（2026-09-03 标注；11.28 Rust 宏层实测收敛）**：Rust 侧 SC_JOIN_* 宏内部为晶体管级 PUN/PDN（M_ 行，med 5 管），名称编码逻辑结构（如 SC_JOIN_AND_AND = 4 管 AND2）；V3 数据生成沿用 Rust `expr_to_hierarchical_spice` 即产出同套宏，无需另行定义。**V3 的 `sc_expansion.json`（R4 已列）需覆盖 V3 宏集（预计数百~数千种，格式为宏→内部逻辑/晶体管结构）；具体格式待与训练端 STRUCT_MODE 用法对齐后定稿**。方案 A（LIB 查表）未启用（优先级低于 B），不影响当前交付。
>
> **关键前置条件：SC_ 宏展开为标准单元**
...
不再独立为 `data/batch_wave/`。**V3 单一数据集：每一行的 `timing_arcs.parquet` 都必须包含此列**（无 batch1/1b/2/3 分批）。
...
1. **每一行**都必须有非空 `transistor_wave_json`——**不允许「只在部分 corner 采样」或「部分电路提供」**。
...
> **V3 单一数据集：单批 ~60 万行**（单 corner + 任意 I/O + 每组 10-15 变体），数量与配额见第四节。V1 的「两阶段（60 万升级 + 120 万新增）」与 V2 分批（full/rest/m4）均已废弃。
...
生成方在启动 60 万行全量前，**先完成小批预检（建议 5-8 个电路：★ 档位大/深形状如 (9,6)/(7,4)/(8,3) 级 ≥1 个 + 小形状、多输出各 ≥1 个；单 corner 2ps/1fF 仿真 + 全部列输出）**，并附 `data/coverage_report.json`。接收方立即逐字段核对：
