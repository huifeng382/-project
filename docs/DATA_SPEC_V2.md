...
> **⚠ 锚定形状对估算的影响（V3，2026-09-03）**：下方「### I/O 形状锚定」要求 Rust 18 形状各 ≥200~1500 电路（合计 ~7900，其中 ★ 多输出大形状行成本高——如 (9,6) 每电路 2×9×6=108 行）。多输出占比显著高于旧数据 → 平均每电路行数 > 12，**实际电路数将低于 5 万**。行数预算以 **60 万行封顶**，电路数按实际行数反推，不再死守 5 万/4000 组。
...
### I/O 形状锚定（Rust 18 实测形状全覆盖 + 每形状规模档配额）——V3 新增 2026-09-03

> **为什么必须锚定（上轮 m4 实测教训，2026-09-02/03）**：旧 V2 m4（1800 电路）特意选了 Rust 的 5 个多输出形状 (4,3)(5,5)(7,4)(8,4)(9,6)——**形状选对了，但每个形状内的实现全部是「最小规模缩水版」**（实测）：
>
> | m4 形状 | m4 实测 trans（med/max） | Rust 对应电路 | Rust 实测 trans（med/max） | 差距 |
> |---|---|---|---|---|
> | (4,3) | 26 / 30 | ENC4 | 56 / 69 | ~2× |
> | (5,5) | 52 / 56 | SHIFTER4 | 48 / 60 | ✓ 达标 |
> | (7,4) | 58 / 66 | ALU2 | 235 / 262 | ~4× |
> | (8,4) | 66 / 74 | ENC8 | 189 / 233 | ~3× |
> | (9,6) | 110 / 118 | ADD4_OVF | 247 / 316 | ~2.2× |
>
> rest 同类：（8,3）max 182 < Rust COMP4 233；（5,2）max 158 < Rust ALU_SLICE_SMALL 208。**18 种 Rust 形状旧数据其实全量覆盖（形状不缺），但每种大形状内没有达到 Rust 档位的实例 → Rust 深/大电路（ADD4_OVF/ALU2/ENC8/COMP4）在训练中仍是规模+深度 OOD。** 根因 = 旧规格只写「多输出 ≥20%」这类**比例**约束，生成方凑够比例即可、每形状都取最简实现。**V3 改为形状-规模锚定（电路数配额 + 规模档 + 功能锚定），并用 metadata 逐形状验收，单一占比统计不再作为验收依据。**

**锚定清单（Rust 46 电路 18 种实测形状；trans 区间为 Rust 实测 2026-09-02）：**

| (N_in, M_out) | Rust 代表电路 | Rust trans 实测 | 类别 | V3 最低电路数 |
|---|---|---|---|---|
| (1,1) | INV1 | 4~8 | 普通 | ≥200 |
| (2,1) | OR2/AND2/XOR2/XNOR2/HA_CARRY/HA_SUM/EQ2 等 7 电路 | 8~51 | 普通 | ≥300 |
| (2,2) | HA | 26~36 | 普通（多输出） | ≥250 |
| (2,3) | COMP1 | 29~32 | 普通（多输出） | ≥200 |
| (3,1) | OAI21/AOI21/MUX2/MUX2I/MAJ3/FA_COUT/RCONV1/XOR3 等 9 电路 | 11~39 | 普通 | ≥300 |
| (3,2) | FA/CSA_3_2 | 49~50 | 普通（多输出） | ≥300 |
| (4,1) | AOI22/OAI22/OR4/AND4/PARITY4/SHARE1/2/RCONV2/EQ4 等 10 电路 | 12~52 | 普通 | ≥300 |
| (4,3) | ENC4 | 56~69 | 中规模（宽松下限，曾缩水） | ≥350 |
| (5,1) | AOI221/OAI221/**DEPTH_MIX** | 14~117（深） | **★深度档** | ≥1500（深链为主） |
| (5,2) | ALU_SLICE_SMALL | 140~208 | **★规模档** | ≥800 |
| (5,5) | SHIFTER4 | 48~60 | 普通（多输出） | ≥250 |
| (7,4) | ALU2 | 235~262 | **★规模档** | ≥500 |
| (8,1) | AND8/OR8 | 22~67 | 普通 | ≥250 |
| (8,3) | COMP4 | 170~233 | **★规模档** | ≥600 |
| (8,4) | ENC8 | 189~233 | **★规模档** | ≥450 |
| (9,1) | OVF/ovf1 | 104~146（深） | 中规模（建议含深链） | ≥400 |
| (9,6) | ADD4_OVF | 247~316（深） | **★规模档** | ≥300 |
| (16,1) | AND16 | 35~40 | 普通 | ≥250 |

> ★ = 强制爬 Rust 规模/深度档（见下）。普通/中小形状行数便宜；★ 多输出大形状行数贵（每电路 2×N_in×M 行），电路数与行数预算自行平衡，**总量以 60 万行封顶**。

**强制规模档（★ 大形状；防 m4 重演——「形状存在」不算数，形状内必须有达到 Rust 档位的实例）：**

> - ★ 五大规模形状 (5,2)(7,4)(8,3)(8,4)(9,6)，晶体管分布**整体爬到 Rust 实测区间**，不得整体偏小：
>   - 形状内 trans **p50 ≥ Rust med**（如 (9,6) 的 p50 ≥ 247；(7,4) ≥ 235；(8,4) ≥ 189）；
>   - 形状内 **max ≥ Rust max × 0.85**（如 (9,6) 至少要出现 ≥ 268 管的电路）；
>   - 禁止 m4 式「同形状只有 60~110 管的小实现」。
> - **★ 深度档 (5,1)**：形状内 ≥30% 电路最长门链深度 ≥ 8，整体 depth_p90 ≥ 10（深链为主，15-25 门串 8-15 层，DEPTH_MIX 型；不是加宽）。OVF/ADD4_OVF Rust 实测深 8-12，(9,1)/(9,6) 亦应含深链变体。
> - **宽松下限（所有多输出锚形状，防 (4,3) 式缩水）**：非 ★ 多输出形状 ((2,2)(2,3)(3,2)(4,3)(5,5)) 的 trans p50 ≥ Rust med × 0.7（如 (4,3) p50 ≥ 39，禁止 m4 的 26-30）。
> - **功能锚定（★ 形状必须按 Rust 功能族生成，否则 e-graph 枚举不出大电路）**：(9,6) → 4-bit ripple-carry 加法器 + 溢出/进位族；(7,4) → 4-bit ALU/算术族；(8,4) → 8→4 优先级编码族；(8,3) → 4-bit 比较/复合族；(5,2) → ALU-slice 族；(5,1) → 深组合链族。判据：每输出依赖 ≥4-5 个不同输入；多输出共享中间节点（禁止「6 个输出各算各的」拼形状）；以 Rust 对应电路功能等价类为种子做 e-graph 变体化。**e-graph 枚举后按规模下界过滤（trans < Rust med × 0.5 的丢弃），不凑数。**
> - 非 ★ 形状不做规模强制（旧数据已充分覆盖小形状），但 18 形状每形状电路数 ≥ 表内最低数（保底不缺失）。

**量化验收（metadata.json 新增 `shape_coverage`）**：逐形状记录 `{n_circuits, trans_p50, trans_p90, trans_max, depth_p90}` + Rust 参考档（代表电路 / trans med / trans max），验收规则：
>
> 1. 18 锚形状 `n_circuits` ≥ 表内最低数；
> 2. ★ 五大规模形状 `trans_p50 ≥ Rust med` 且 `trans_max ≥ Rust max × 0.85`；
> 3. (5,1) `depth_p90 ≥ 10` 且 ≥30% 电路深度 ≥ 8；
> 4. 所有多输出锚形状 `trans_p50 ≥ Rust med × 0.7`；
> 5. **形状 + 规模/深度双达标才算该形状合格**——「多输出占比 ≥20%」等单一比例统计不再作为验收依据。
