...
| **组内延迟差** | 相邻变体差 med 0.03%、<1% 占 82%（train/val/test 一致）——全挤超近差 | **组内变体延迟差多档覆盖**（<1% / 1-5% / 5-20% / >20% 各档，≥80% 组含 ≥2 个差 >10% 变体（2026-09-03 对齐 Rust 实测 81%，见锚定小节）） |
...
> **⚠ 锚定对估算的影响（V3，2026-09-03 修正）**：下方「### 形状-规模-深度锚定」Tier A（18 形状合计 ~7900 电路 ≈22.2 万行 ≈**37% 行**；★ 多输出大形状行成本高——如 (9,6) 每电路 108 行）+ Tier B（Rust 邻域 ≈63% 行）。多输出占比显著高于旧数据 → 平均每电路行数 > 12，**实际电路数将低于 5 万**。行数预算以 **60 万行封顶**，电路数按实际行数反推；Tier A/B 占比按行数核算。
...
> 5. **变体延迟差多档覆盖（16.11.20 修正——原「补近延迟对」不成立；2026-09-03 验收口径对齐 Rust 实测）**：实测训练组内相邻变体差 med 0.1%（93% <5%），**不缺近延迟对**。真正问题是模型未学会排序（训练目标），不在数据。为防组内变体**全挤在 <1%**（无排序信号梯度），要求组内延迟差**多档覆盖**。**验收线（Rust 714 有效候选集实测 2026-09-03）**：① **≥80% 的 expr 组含 ≥2 个与组内最优差 >10% 的「拉开距离」变体**（对齐 Rust 实测 81% 组自然满足；不强求每组 100%——Rust 19% 近差功能组天然达不到，排序难度低，强制会逼生成方硬造不真实变体）；② 组内相邻差四档（<1% / 1~5% / 5~20% / >20%）全局分布各档均非零（对齐 Rust 实测 20/35/39/6%）。
>
> **量化验收**：交付 metadata.json 必须含 **① transistor_count 分位数（p50/p90/max）；② 深度分位数；③ 组内变体延迟相邻差分布（<1% / 1-5% / 5-20% / >20% 各档占比）；④ 弱驱动中间门占比（判据见上第 4 条）**；并附 `shape_coverage` 与 Tier A/B 占比（见「### 形状-规模-深度锚定」验收），供接收方核对上述配额达标。
