...
| **门表达** | 巨型复合门 SC_JOIN_* 把整电路压成 ~15 个顶层 X_（m4：trans 68/顶层 15 门 vs Rust 候选 33-57 个） | **SC_JOIN_* 宏层次 + 宏粒度对齐 Rust（2026-09-03 修正）**：与 Rust `expr_to_hierarchical_spice` 输出同构（dut .sp 实测）；每宏 2-8 逻辑（trans ~4-8 管）、X_ 数与 Rust 候选同量级；禁止超大宏打包（宏粒度是深度/规模验收可比的前提） |
...
> - **Tier B · Rust 邻域多样化（~63% 行）**：不锚定具体档位，但**不脱离 Rust 使用域**——形状 ∈ Rust 可达范围（1~16 入 / 1~6 出）、trans ≤ ~400（316-400 少量，泛化余量）、深度 ≤ ~18；**分布判据（2026-09-03，防退回旧 rest 浅小铺量）**：Tier B 内 trans p50 ≥ 120 且 140-190 管过渡带 ≥20% 行、深度 6-8 过渡带 ≥15% 行（衔接 Tier A 档位，保证邻近密度）；禁止 Rust 域外形态（超深无意义链、超宽纯拼接、奇异多输出如 (16,3) 式）。
...
> 3. **宏粒度对齐（2026-09-03 修正——原「用基本门显式展开」表述与 Rust 实测相反）**：Rust 候选 dut .sp 实测为 SC_JOIN_* **宏层次**（非基本门展开）。V3 沿用宏层次（同 `expr_to_hierarchical_spice`），但**宏粒度对齐 Rust**：每宏 2-8 逻辑（trans ~4-8 管）、X_ 数与 Rust 候选同量级（ADD4_OVF 类 ~50 个 X_）；禁止 m4 式把整电路压成 ~15 个超大宏。宏粒度不一致会使「X_ 宏级深度 / trans」验收失真。
...
> ⚠ **待复核（2026-09-03）**：Rust 侧 SC_JOIN_* 宏（dut .sp）内部为晶体管级 PUN/PDN 直接实现（M_ 行），非「ASAP7 标准单元互联」；本方案假设与 Rust 宏语义是否一致待验证。方案 A 未启用（优先级低于 B），不影响当前交付。
>
> **关键前置条件：SC_ 宏展开为标准单元**
