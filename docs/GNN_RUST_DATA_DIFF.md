...
- **DEPTH_MIX（39 管）规模不 OOD**——它在训练范围内（52-58 中位）但 recall 低 → OOD 在**深度/结构**（§12.4 缺陷 1），非规模。
...
- **说明**：DEPTH_MIX(39 管)规模在训练范围内但 recall 低 → 其 OOD 在**深度/结构**（缺陷 1），不在规模。
