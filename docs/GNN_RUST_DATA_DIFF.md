...
## 十六、90.8% 与 GNN-only 问的不是同一件事（2026-09-20）

> **本节的用途：给「90.8% 严格@3 能不能拿去支持 GNN-only」这个问题一个结构性的否定回答。**
> 实测侧的证据与数字记在 `PROJECT_LOG.md`「GNN 作为 tl_opt 决策门为何失效」的 **(13)**（2026-09-20 复核），
> 此处只记**口径层面**的结论，避免两处各写一份。

### 16.1 两套的「批」内容不同

| | shadow 粗排（90.8% 的出处） | GNN-only（决策门） |
|---|---|---|
| 批里有什么 | **只有候选**（`gnn_shadow.rs` 的 `pre_rank` 只打包 `modules`，`current` **不在**批里） | `current` 与候选**同批**（`gnn_only.rs` 的 `rank_window`，`current` 的 id = `CURRENT`） |
| 谁做接受决定 | **SPICE**（top-K 精排 → 接受第一个改善的） | **模型自己**（`best_cand_score < cur_score`，严格 `<`） |
| 要授权的设计 | §「优化改成 GNN 排全部候选 → top-K 才 SPICE 精排」那条 | 「搜索过程零仿真」 |

⇒ **两者唯一新增的能力要求就是「该不该动」这一问**（`current` vs 候选），而 shadow 那一套**从不问它**。

### 16.2 但「候选之间」这一层两套是同一个东西 —— 可以直接对读

每个候选的前向与批内其他成员**无关**（`serve.py` 的批量路径是 `for c in cands` 各自独立前向），
所以**把 `current` 塞进同一批不改变任何候选的预测值、也不改变候选之间的相对序**。
⇒ 「候选内排序」这一层的读数（如 §15.2 的严格@3、选择遗憾）在两套之间**可以搬运**；
**不可搬运的是「`current` 的落位」**。

### 16.3 三样必须同时声明的差异（否则会误读）

1. **分数是组内竞争名次**（`avg = (i + j) / 2.0 + 1.0`，并列取平均），**不是延迟** ⇒ 同一个电路在两套里的数值不是同一个数，**跨集阈值/绝对值一律不可搬**。
2. **池子来源不同**：shadow 的 `current` 是 SPICE 冠军，走步的 `current` 是**模型自己接受出来的** ⇒ 两条轨迹的候选池不同源。
3. **覆盖电路不同**：§15.4 已记 714 集只 **18 个电路**、`DEPTH_MIX` 一家占 **75% 权重**、重复度 **77.5%**；而走步诊断只 **AND2 单电路**。**两边都不构成对方的外推样本。**

### 16.4 最狠的一条：这把尺子**结构上看不见** GNN-only 要问的东西

严格 recall@3 与选择遗憾的分子分母**都取自候选集内部**（遗憾 =（模型选中 − **候选集内**真最优）/ 候选集内真最优）
⇒ **一个「所有候选都比 `current` 差」的窗口在这把尺子上是满分（遗憾 0%）**。
而这类窗口正是 GNN-only 唯一会犯错的地方 —— **判据与风险面正交**。

⇒ 结论：**90.8% 不是「不够好」，是「问的不是同一件事」。** 引用它时必须写明它**只覆盖「候选内粗排」**；
要论证 GNN-only 必须用**直接测 `current` vs 候选**的读数（现成的在 `PROJECT_LOG` 该节 (13)）。

### 16.5 ⚠ ckpt 之间不可盲换（`ep250` vs `ep300`）

线上 serve 起于 `midpoint_ep300.pt`（`n_models: 1`）。**不建议盲换成 `ep250`**，理由三条：

1. `ep250` 与 `ep300` 来自**同一次训练**（`project-107-v2nowave42m4`），**不是两批不同产物**；
2. 症状**不是边际的 epoch 回退**（门的判决方向与真值反相关、且输给「数管数」），**换 epoch 改不动这类错**；
3. `ep250` 的 90.8% 从未在 `ep300` 上重测过，「被验证过」**不等于**「更好」—— **§15.8 已定案：历史表的 ckpt 标注不可信**，跨 ckpt 排序必须用 batch 口径逐 ckpt 重测并盖 ckpt sha1（§15.7 待办 2，**仍未做**）。

**最可疑的替代解释是「部署配置与训练不匹配」**（`--scaler` 路径、`USE_IDS_AVG_APPROX` 模式），
其症状恰好是「**批内相对序仍在、绝对水平偏掉**」——与实测吻合（候选之间 τ 中位 +0.485 仍好，而绝对量级差 9×）。
⇒ **先查配置一致性，再谈换 ckpt。**

⚠ **另：不要重启线上 8000 那个 serve**（已连续运行 25 h，`etime 1-00:57:49`，60w 战役可能在依赖它）。
要试就**另起一个 8001**。

### 16.6 待办

1. 把 §16 的结论**并进 §15.6 的读表须知**（同一节里「达标与否」的讨论必须带这句口径限制）。
2. 「部署配置 vs 训练配置」逐项对账（scaler 路径、`USE_IDS_AVG_APPROX`、`OMP_NUM_THREADS`、特征开关）——
   **零仿真、纯配置核对**，优先级高于任何 ckpt 扫描。
3. §15.7 待办 2（各 midpoint 以 batch 口径重测 + 盖 sha1）**仍然有效且未做**；它是 16.5 第 3 条的前提。

---

## 十七、V3 交付接入实测（2026-09-20，先于首次 V3 训练）

按 §12.4 给生成方的要求清单收口后，V3 单一数据集已交付并落到本地工作树。
本节是**接入前置核对**：全部为只读实测（三个脚本在 `scripts/diag/`，均未改数据、未跑训练），
目的不是评价 V3 好不好，而是回答「直接拿它训一次，会发生什么静默退化」。

### 17.1 V3 是什么（已落地本地）

| 项 | 值 |
|---|---|
| 位置 | `data/v3_delivery/`，37 文件 / 2.8 G |
| 远端 | 分支 `10.3.3-fix-earlystop` @ `d2533a0`（本地 `4b39654` 的**直接后代** ⇒ fast-forward，无冲突） |
| 形态 | 单 `circuit_static.parquet` + **31 个** `timing_arcs_partNN.parquet`（混合） |
| 规模 | **599,976 行 / 2,903,619,755 字节** —— 逐片比对 `PARTS.txt`，**0 处不一致** |
| 附带 | `metadata.json`、`coverage_report.json`、`sc_expansion.json`（1,049 宏）、`timing_arcs.sha256`、`PARTS.txt` |
| 电路 / 组 | 12,455 电路 / 837 expr 组；组大小 `{10:3, 11:14, 12:3, 13:6, 14:8, 15:803}`，**无 <10 的组** ⇒ `MIN_GROUP_SIZE=10` 不剔任何组 |

⚠ **不要试图靠 `cat` 分片拼回单文件**：已验证（pyarrow 24.0.0 / pandas 2.2.3）
抛 `OSError: Couldn't deserialize thrift` —— 分片是 **snappy 重写**，不是原文件的字节切片。
`~/NetlistOpt/data/v3_delivery/timing_arcs.parquet`（3.94 G 原始单文件）据交付文档仍在，
但 `~` 归属有歧义（生成方工作目录 `/home/zhirui/…` vs 训练方 `tianlang`）⇒ **不作依赖**；
需要单文件就重新生成。当前 loader 直接读多文件，**不需要**单文件。

### 17.2 两个接头差异（不处置 = 静默退化，两者都不报错）

#### (a) loader 的形态组合假设漏了「混合」

`src/train_sweep.py` 旧写法要求 static 与 arcs **两侧同时**是单文件、或**同时**是分片。
V3 是「单 static + 31 分片 arcs」⇒ 两侧都不匹配 ⇒ 打印 `V2 data not found, skipping: v3_delivery`
然后**一个文件都不加载**。`DATA_SPEC_V2.md`「parquet 可拆多文件但同属一集」⇒ 混合形态合法，
是 loader 的组合假设漏了，不是数据不合规。

**修法**：两侧**各自独立**判定（单文件优先，否则取分片），抽成模块级
`resolve_v2_batch_files(data_dir, data_batches)`。抽成模块级是为了让验收脚本调用**这份真代码**
而不是另抄一份逻辑（另抄一份等于只测了副本）。

**验收（`_t_v3_loader_check.py`，只读）**：
- `v3_delivery` → `(1 static + 31 arcs)` ✅
- V2 默认三批**逐字节不变**：`batch_v2_full (1+1)`、`batch_v2_rest (1+6)`、`batch_v2_m4 (1+1)` ✅
- 负例仍 `skipping` 而非崩溃 ✅
- 单文件与分片**并存**时只取单文件且**出声告警**（可疑状态不许静默丢一半数据）

#### (b) 宏表读的是硬编码共享路径，而两张表的名域**近乎互斥**

`graph_builder._load_sc_expansion()` 固定读 `data/sc_expansion.json`，V3 自带
`data/v3_delivery/sc_expansion.json`。按规格 P15（宏表须覆盖训练数据里出现的**全部** SC_ 名），
**两个方向都必须测**：

| 表 | 条目 | 对 V3 名域（1,049 个） | 对 V2 名域（3,586 个） |
|---|---|---|---|
| V3 自带表 | 1,049 | **1,049/1,049 = 100.0%** | 12/3,586 = **0.3%** |
| 当前共享表 | 24,625 | 87/1,049 = **8.3%** | **3,586/3,586 = 100.0%** |

⚠ **只测 V3 那一列会得出错误结论**（本报告初稿就只测了一列，结论被下面这次补测推翻）。
两条路**都不通**：

* **只覆盖**（把共享路径换成 V3 表）⇒ **V2 数据加载被打崩**（可展开率 100% → 0.3%），
  V2 的复现性直接毁掉；
* **只保留** ⇒ V3 有 **91.7%** 的 SC_ 宏落进 `gate_struct` 的兜底
  （`logic='COMPLEX', n_t=6.0, drive=1.0, stack=1.0, parallel=1.0`），
  `STRUCT_MODE='base'` 的 n_transistors 特征近乎常数 —— 正是 §7.3 记过的「候选间差异被抹平」形状。

**已定：取并集**（`scripts/merge_sc_expansion.py`）。实测 union **25,587 条**
（base 24,625 + 净增 962），两个名域**都 100.0%**；**87 个同名条目内容完全一致（0 处冲突）**
⇒ 合并无歧义，且对 V2 是**逐字节等价**（同名条目原样保留、V2 名域 3,586 个全部命中）。
`lib_lookup.load_expansion` 只被死代码 `train_lib.py` 调用，故共享路径覆盖所有**实际**读取方。

**落在哪**：**不在仓库里提交合并表**，而是在 `setup_exp.sh` 的 `v3*` 变体里**当场合并进 run 树**
（`$D/data/sc_expansion.json`）。这样仓库的共享表保持 V2 内容 ⇒ **V2 运行零风险**，
合并只写在一次性 clone 的 run 树里、不影响任何既有产物。合并失败**拒绝启动**
（不允许带着 8.3% 覆盖率的表静默训练）。

⚠ **副作用（已记档）**：仓库里那份表对 V3 名域**仍只有 8.3%** ⇒ 任何**不走 `setup_exp.sh`** 的
V3 消费者（本地诊断脚本、Rust 侧、手工跑）必须先自己合并一次：
...
**后续可选（本次不做）**：合并产物是 **60 MB**，比原始 93.9 MB **更小**（原表带缩进，`json.dump`
是紧凑写法）⇒ 直接提交合并表在体积上反而省 34 MB，且能让所有消费者一次到位。
未采纳的理由是**改动面**：那会改写生成方的共享数据文件，收益（覆盖非 `setup_exp.sh` 路径）
在首次 V3 运行前不是必需的。记为独立课题。

### 17.3 schema 与空值：V3 = V2 的**严格超集**，无缺列

- static 13 列 = V2 10 列 + `shape`、`split`、`tier`；arcs 17 列 = V2 16 列 + `split`。
- **V2 独有列 = 无**（缺列才会让特征静默退化，逐列核过）。
- 31 片 schema **完全一致**（不一致会让 `concat` 错列）。
- 训练必需 arcs 列（`circuit_id/expr/switching_pin/direction/DELAY/vector/slew_s/output_load_f`）
  与 `transistor_wave_json` 的**空值均为 0**。
- ⚠ 空值取自列统计；**统计缺失记「未验证」而不是 0**（未知 ≠ 无空值）。

### 17.4 独立复核交付方 GATE 7 —— 并**撤回我自己先前的一个警报**

口径写死在脚本里避免读法漂移：每个电路的标量延迟 = 该电路全部行 `DELAY` 的**均值**
（与 `truth2.csv` 的 `avg_delay` 同口径）；组内升序排序后取相邻相对步长与组跨度 `(max-min)/median`。

**逐格复现交付方的 GATE 7（V3）**：相邻步长 `<1%` = **67.3%**、`1-5%` = 15.9%、
`5-20%` = 12.2%、`>20%` = 4.7%；步长 p50 = 0.0000、p90 = 0.1177；
宽松（>1%）32.7%，严格（>5%）16.8%；组内跨度中位 **0.195**，跨度 >10% 的组 68.8%、>30% 的 42.3%。
⇒ **与交付方自评数字逐格一致**，这提高了对其余条目（含 `transistor_count` 口径更正）的信任。

⚠ **但「分布窄 = V3 数据变差」是我先前的误读，此处撤回。** V2 自己就是这个形状：

| 数据集 | 相邻步长 <1% | 组内跨度中位 | 跨度 >10% 的组 |
|---|---|---|---|
| V3 | 67.3% | 0.195 | 68.8% |
| `batch_v2_full` | 64.8% | 0.296 | 84.8% |
| `batch_v2_rest` | 67.4% | 0.151 | 61.5% |
| `batch_v2_m4` | 83.6% | 0.053 | 12.7% |

⇒ **V3 与 V2 属同一量级，不是更差**（`m4` 甚至窄得多）。「组内区分度不足」是这条数据线
一直以来的形状，不是 V3 引入的回归。引用 GATE 7 时必须**两个都写**，否则会得出错误结论。

### 17.5 窄跨度**集中在 9x6**（这才是 V3 的新东西）

按 `shape` 分解组内跨度中位（电路数降序）：
...
| 9x1 | — | — | 0.299 |
| 5x2 | — | — | 0.364 |
| 3x2 / 5x5 / 4x3 / 2x3 | — | — | 0.502 / 0.534 / 0.731 / 0.775 |
| 16x1 / 4x1 / 3x1 / 8x1 | — | — | 0.813 / 0.846 / 0.887 / 0.896 |
| 2x2 / 5x1 / 2x1 | — | — | 1.119 / 1.127 / **1.449** |

9x6 占交付行的 **71%**，而它的跨度中位只有 **0.080** —— 全交付的窄分布主要由它一个形状贡献。
⇒ 若首次 V3 训练的整体排序指标不好看，**先按 shape 分层读**，别把 9x6 的性质当成全体性质。

### 17.6 ⚠ `struct_prior` 第 2 列在 V3 上**彻底死掉**（第 3 列在 V2 就死了）

`data_loader.py:715-719` 用**写死的 V2 时代名字匹配**构三列：
`[transistor_count, #SC_AND, #SC_INV_WIRE]`。`model.py:50` 用
`nn.Linear(3, hidden_dim//4) + ReLU` **直接吃原始值**（无 scaler、无归一化）。实测：
...
根因：V3 的命名域是 1,049 个名字，主体为 `SC_INV`、`SC_JOIN_AND_WIRE_WIRE_AND_WIRE_WIRE`、
`SC_JOIN_AND_AND` … —— **没有裸 `SC_AND`**（V2 的 3,586 个名字里 `SC_AND` 出现 1,952 次）。
匹配式 `'SC_AND' in g and 'SC_AND_' not in g` 在 V3 上恒为 0，**静默**：不报错、不告警，
只是模型永远学不到那一维。

**两点必须分清**：
1. 这**不是 V3 引入的**缺陷 —— 三列结构先验长期只有约 1 维有效（V2 已有 77.4% 退化），
   且 `#SC_INV_WIRE` 在 V2 就是 99.9% 零；
2. **V3 把唯一还有点活的 `SC_AND` 也清零了**（退化比例 77.4% → 89.6%）。
   ⇒ 若首次 V3 训练要归因结构特征，先说清它实际只有 `transistor_count` 一维在工作。

**待决**：是否把匹配式改成 V3 名字域（`SC_JOIN_*` 重组）. 改 = 特征集变化、与全部既有 V2 运行
不可比；不改 = 保持可比但承认该维无效。**首次 V3 运行建议不改**（先取得一次可比读数），
把「结构先验重构」记为独立课题。

### 17.7 ⚠ `transistor_count` 口径平移约 4×，而它是**未归一化的输入特征**

交付方 round 10 声明换过口径：V3 按**实例展开**计数（`TRANS_PER_INSTANCE`，与
`transistor_wave_json` 键数一致），V2 是**宏去重**计数
（`re.findall(r'^M', text, re.M)`）。本地实测：

| 数据集 | p10 | **p50** | p90 | max |
|---|---|---|---|---|
| V3 | 40 | **208** | 329 | 400 |
| V2 `batch_v2_full` | 42 | **52** | 66 | 96 |

（此差混合了口径与形状构成两种因素，不全是口径所致。）

⇒ 该列同时是 `data_loader.py:322` 的输入特征与 `:716` 的 `struct_prior[0]`，且**无归一化**。
从零训练不会有「旧 ckpt 迁移」问题，但**输入量级整体上移约 4×**，进 `nn.Linear(3,…)+ReLU`
的初始激活尺度随之变化。**记为观察项**（首次 V3 训练若 loss 前期异常，先查这里），
不在本次改动范围内。

### 17.8 训练用的是自切划分，**不用交付的 `split` 列**

`train_sweep.py:449` 调 `split_by_expr(circuit_ids, id_to_expr, seed=SPLIT_SEED)`（`SPLIT_SEED=42`，70/15/15）。
V3 新增的 `split` 列（交付方 431,439 / 62,835 / 105,702）**当前无任何读取方**。
⇒ 我们的行数**不会**复现交付方的划分统计量，对比时别拿它当同一把尺子。

### 17.9 启动路径：已补什么 / 仍缺什么

**已补（18.0.0，本次）**：

1. `setup_exp.sh` 新增 `v3wave<seed>` / `v3nowave<seed>` 两个臂，变体自己
   `export DATA_BATCHES=v3_delivery`（`config.py` 在 import 时读 env，`nohup python3 main.py` 继承）
   ⇒ **不必**手工传 env。未知 `v3*` 名**报错退出**，不静默落回默认 wave。
2. 宏表在 `v3*` 变体里当场合并进 run 树（§17.2(b)）；合并失败**拒绝启动**。

**仍缺（如实记档，不影响首次运行）**：

3. **⚠ 变体名绝不能复用既有 V2 名**：`:28` 先 `rm -rf "$D"`，`$D=~/project-107-$V`
   ⇒ 用 `v2wave42` 跑 V3 会**删掉已有 V2 运行目录**。必须用 `v3*` 名。
   而 `v3nowave42` 这个名字**不会**命中 `v2nowave[0-9]*` 分支（该模式只匹配字面 `v2`）——
   补的显式臂正是为此，不能靠名字「看起来像」。已逐名路由验证：`v3wave42` / `v3nowave7` /
   `v3nowave42m4` 全中对应臂，无 seed 的 `v3wave` 与未实现的 `v3iaa42` 落 `v3*` 错臂并 `exit 1`。
4. **`CACHE_SEED` 对 V3 失效**：播种块只 `cp -a` 四个 `batch_v2_*` 目录，不含 `v3_delivery`
   ⇒ 复制了 cache 但**不复制数据/不保 mtime**，而缓存有效性检查吃 mtime
   ⇒ 第二趟起缓存会被判失效、清空重建。**首次 V3 运行无此问题（本来冷 cache）**。
   ⚠ `setup_exp.sh:253` 的注释已把 `~/project-107-v3wave42` 当 CACHE_SEED 示例
   ⇒ 这个坑**必定会被踩**；第二趟前需把 `v3_delivery` 加进播种列表。
5. **`git clone` 无 `--depth`** ⇒ 每次新 V3 运行都完整拉取 2.9 G 数据（首次即冷建图缓存，
   启动慢属预期，不是卡死）。
6. **仓库共享表对 V3 名域仍只有 8.3%**（§17.2(b) 副作用）⇒ 非 `setup_exp.sh` 的 V3 消费者需手动合并。

### 17.10 本节的可复现脚本

| 脚本 | 作用 |
|---|---|
| `scripts/diag/_t_v3_loader_check.py` | 调**真** `resolve_v2_batch_files` 验收 (a)；分片行数/字节 vs `PARTS.txt`；schema 三方对照；空值；宏表覆盖（**双向**，且支持 `SC_EXPANSION_EXTRA=…` 验收并集表）；`transistor_count` 分布 |
| `scripts/diag/_t_v3_group_spread.py` | 独立复核 GATE 7（17.4/17.5），含 V2 三批对照与逐 shape 分解 |
| `scripts/diag/_t_v3_struct_prior.py` | 逐字复算 `data_loader.py:715-719` 的两个匹配式（17.6），打印命名域对照 |
| `scripts/merge_sc_expansion.py`（工具，非诊断） | 宏表**并集**（§17.2(b)）。默认只报数不写；`--out` / `--inplace` 才写。幂等 |

三个脚本均只读 `data/` 与仓库代码，**不改文件、不跑训练**。

---

## 十八、V3 是否满足当初给生成方的要求（2026-09-20，逐条验收）

**触发**：用户要求「检查一下这个版本的数据是否满足之前的数据生成要求」。
**依据**：正式规格 `docs/DATA_SPEC_V2.md` §四（尤其「### 形状-规模-深度锚定」及其「量化验收」
A 段 7 条）；`§12.4` 的六缺陷清单为**分析依据**（其中第 1/2 条 2026-09-03 已降为「统计参考」，
判定以 §四为准）。**复现脚本**见 §18.9，全部只读。

### 18.1 结论摘要

| 结果 | 项数 | 是哪些 |
|---|---|---|
| ✅ 达标（含超额） | 12 | 深度、规模、★/◆ 两档、形状内的规模/深度锚定、样本量、组大小、Tier 占比、Rust 域外、单 corner、行数上限、sc_expansion 覆盖 |
| ❌ 未达标 | 7 | 形状配额（1x1 缺失 + 5 个单输出形状不足）、Tier B 两条过渡带判据、I/O 分桶等比例、缺陷 4 三条硬指标（组内延迟差）、交付配套缺深度分位 |
| ⚠ 达标但存疑 | 1 | 弱驱动中间门占比（判据近于恒真，见 §18.6） |

**一句话**：**V2 两轮失败的三条根因（浅、小、形状内规模缩水）全部被解决，且是超额解决**；
但**批次内的分布平衡没做到** —— 整批质量压在一个角上（9x6 大而深），
旧数据原来独有的「浅/小」主流区间只剩 3.7% 的行（§18.5）。**组内延迟差（缺陷 4）基本没改善。**

### 18.2 逐条验收

| # | 要求（出处） | 实测 | 判定 |
|---|---|---|---|
| 1 | 深度覆盖（缺陷 1；§四第 2 条） | p10 **5** / p50 **10** / p90 **13** / max **18**；深 >6 = **81.5% 行**；深 >18 = **0%**。V2 对照：**全 ≤5、>6 = 0%** | ✅ 超额 |
| 2 | ★深主档 (9,1)(9,6)(8,3)(7,4)：≥30% 电路深 ≥9 且 depth_p90 ≥10 | **100% / 100% / 56% / 73%**；depth_p90 **15 / 13 / 11 / 12** | ✅ |
| 3 | ◆深次档 (5,2)(4,1)：≥20% 电路深 ≥7 | **91% / 41%** | ✅ |
| 4 | (9,1) 含 ≥150 个深 ≥10；(4,1) 含 ≥80 个深 ≥7 | **450 个**（该形状 100% 深 ≥9）/ **176 个** | ✅ |
| 5 | 超大电路：180~350+ 管，训练上限 184 → 350+（缺陷 2） | 180-400 管 = **88.9% 行 / 61.3% 电路**；max **400**。V2 对照：**>184 = 0%** | ✅ 超额 |
| 6 | ★规模五形状 p50 ≥ Rust med 且 max ≥ Rust max×0.85 | 7x4 **346**/400、8x3 **212**/233、8x4 **211**/232、9x6 **284**/400、5x2 **183**/207 —— **五形状全过** | ✅ |
| 7 | 189-233 与 233-316 两档均有 | **2,504 / 3,320** 电路 | ✅ |
| 8 | 多输出非★/◆ 宽松下限 p50 ≥ Rust med×0.7 | (2,2)(2,3)(3,2)(4,3)(5,5) 全过（4x3 52 ≥ 56×0.7） | ✅ |
| 9 | 弱驱动中间门 ≥30%（★深/大档内，缺陷 3） | **100%** | ✅ ⚠ 见 §18.6 |
| 10 | ≥80% 组含 ≥2 个与组内最优差 >10% 的变体（缺陷 4 ①） | **63.8%**（>20% 版本 48.0%） | ❌ |
| 11 | 相邻差四档 20/35/39/6 ±10pp（缺陷 4 ②） | **67.3 / 15.9 / 12.2 / 4.7** —— 只有 >20% 档落区 | ❌ |
| 12 | 组内 spread 中位 30-60%（缺陷 4 ③） | **0.195** | ❌ |
| 13 | 18 形状全覆盖 + 逐形状最低电路数（缺陷 5） | **17/18**；6 项不合格（§18.4） | ❌ |
| 14 | ~60 万行；各 expr 组 ≥10 变体（缺陷 6） | **599,976 行 / 12,455 电路 / 837 组**；组大小 min 10 / p50 15 / max 15 | ✅ |
| 15 | I/O 分桶 1~2 / 3~4 / 5~8 / 9~16 各约 25% | 电路 **6.5 / 12.7 / 34.0 / 46.7%**；行 **1.1 / 2.9 / 22.3 / 73.8%** | ❌ |
| 16 | Tier A/B 行占比 ~37/63 | **37.0 / 63.0%** | ✅ |
| 17 | Tier B 判据：trans p50 ≥120、140-190 带 ≥20% 行、深 6-8 带 ≥15% 行 | **277** ✅ / **0.0%** ❌ / **1.0%** ❌ | ❌ |
| 18 | Rust 域外（trans >400、深 >18、奇异多输出）<5% 行 | **0% / 0%**；表外形状 **0 种**；无 >16 入或 >6 出 | ✅ |
| 19 | 宏粒度 X_ 数落 Rust 量级（med 32 / p90 57 / max 73） | med **46** / p90 **68** / max **90** | ⚠ 偏大 |
| 20 | 每组 10-15 变体；每电路行数 ≤2×N_in×M；单 corner | 全过；单 corner `s02p0_l01p0` | ✅ |
| 21 | 交付配套：metadata 含 trans 分位 + **深度分位** + 相邻差分布 + 弱驱动占比；深度字段「若管线能算则直接给」 | 有 trans 分位 / 相邻差 / 弱驱动；**无深度分位**、**无深度列**、**无 §18.2#10 那个指标** | ❌ |
| 22 | `sc_expansion.json` 覆盖全部 SC_ cell | 1,049 宏，对 V3 名域 **100%**（需并集，见 §17.2(b)） | ✅ |

### 18.3 三条根因：已解决（这是本次交付的主要价值）

| V2 的根因 | V2 实测 | V3 实测 | 结论 |
|---|---|---|---|
| ① 深度 OOD | 深度全 ≤5，>6 占 **0%** | p50 10、>6 占 **81.5% 行**、★四形状全达标 | **解决** |
| ② 规模 OOD | trans 上限 184，>184 占 **0%** | 180-400 管占 **88.9% 行**，max 400，★五形状全达标 | **解决** |
| ③ 形状内规模缩水（m4 教训） | (7,4) 58-66 vs Rust 235-262（~4× 缩水） | (7,4) p50 **346**、max 400 | **解决** |

### 18.4 未达标项（逐项证据）

**形状配额（缺陷 5）—— 6 项不合格，全部是单输出形状 + 1x1：**

| 形状 | 交付电路数 | 规格配额 | 差 | 备注 |
|---|---|---|---|---|
| (1,1) | **0** | 300 | −300 | **形状完全缺失**（INV1，4~8 管） |
| (2,1) | 261 | 400 | −139 | |
| (3,1) | 392 | 400 | −8 | |
| (5,1) | 732 | 800 | −68 | |
| (8,1) | 296 | 300 | −4 | |
...
逆方向同样触目：**(9,6) 交付 5,068 个 vs 配额 350 = 14.5×**；(7,4) 794 vs 550。
即**多输出大形状把行预算吃光，单输出形状被挤到配额线下**。
(4,1) 430 ≥ 400、(9,1) 450 ≥ 450 恰好踩线，说明生成方把配额当**目标值**配，
而不是当**下界**——一旦行预算被 9x6 吃掉就轮到普通形状欠账。
规格的「★ 档弹性条款」允许「保 ★/◆ 档、削普通形状配额」，但**没有反向条款**（★/◆ 超额时如何回补普通形状），
所以这 6 项按字面就是不合格。

**缺陷 4（组内延迟差）—— 三条硬指标两条半没沾到：**

| 指标 | 实测 | 目标 | 判定 |
|---|---|---|---|
| ≥80% 组含 ≥2 个与最优差 >10% 的变体 | **63.8%** | ≥80% | ❌ |
| 相邻差 <1% 档 | **67.3%** | 10-30% | ❌ 严重超标 |
| 相邻差 1-5% 档 | **15.9%** | 25-45% | ❌ |
| 相邻差 5-20% 档 | **12.2%** | 30-50% | ❌ |
| 相邻差 >20% 档 | **4.7%** | 3-10% | ✅ |
| 组内 spread 中位 | **0.195** | 0.30-0.60 | ❌ |

**每组「次优 vs 最优」差的中位数只有 0.0089（<1%）** —— 即**连组内最容易区分的那一步都极小**。
V2 相邻差 <5% 占 93%，V3 是 83.2%：**有改善，但离目标（<1% 压到 10-30%）差得很远**。
⇒ 「组内变体延迟全挤在一起、排序没有可学信号」这个病**没有治好**。

**Tier B 过渡带（两条判据）**：Tier B 内 140-190 管带 **0.0%** 行（要 ≥20%）、
深 6-8 带 **1.0%** 行（要 ≥15%）。看 §18.5 的联合分布就明白：**这两条带整体就是空的**
（全局 141-190 管也只占 2.6% 行、深 6-8 占 10.3% 行）。规格设这两条判据的用意是
「衔接 Tier A 档位、保证邻近密度」，实测 Tier B 没有起到衔接作用（见 §18.5）。

**I/O 分桶**：规格要求 1~2 / 3~4 / 5~8 / 9~16 各约 25%，实测行口径 **1.1 / 2.9 / 22.3 / 73.8%**。
原因是行成本：大 I/O 多输出电路每电路行数多（(9,6) ≤108 行/电路），
Tier A 又不受分桶约束，于是行分布被 9x6 拽走。

**交付配套**：规格「量化验收」② 明确要求 metadata 含**深度分位数**，实测 metadata 的
`shape_coverage` 只有 `{n_circuits, trans_p50, trans_p90, trans_max, rows, rust_band,
in_band_share, p50_ge_rust_med}` —— **没有 depth 字段，也没有 depth 列**。
所以本节的深度全部是本报告自算的（这正是 §18.9 脚本存在的原因）。
另外 §18.2#10 那条指标（≥80% 组含 ≥2 个 >10% 变体）metadata **没有报**，
它报的是 `groups_with_gt10pct_spread`（spread>10% 的组占比），**两者定义不同**，不能互读。

### 18.5 结构性发现：整批质量压在一个角上（比单项不合格更要紧）

**联合分布（行数口径，本报告实测）：**

| trans \ 深度 | 0-5 | 6-8 | 9-12 | 13-18 | 合计 |
|---|---|---|---|---|---|
| 0-100 | 18,153 | 17,414 | 9,198 | 3,456 | 48,221 |
| 101-140 | 1,315 | 1,520 | 5,636 | 2,750 | 11,221 |
| **141-190** | **168** | **5,300** | 8,190 | 1,928 | 15,586 |
| 191-233 | 2,550 | 28,510 | 81,900 | 30,487 | 143,447 |
| 234-316 | 0 | 4,108 | **227,340** | 38,418 | 269,866 |
| 317-400 | 0 | 4,972 | 77,399 | 29,264 | 111,635 |
| 合计 | 22,186 | 61,824 | 409,663 | 106,303 | 599,976 |

- **一格（234-316 管 × 深 9-12）占 37.9% 的行。**
- **9x6 一个形状 = 5,068/12,455 电路（40.7%）、425,231/599,976 行（70.9%）。**
- **Tier B 的 4,645 个电路里 4,349 个是 9x6（93.6%）**；Tier B trans p50 **277**、depth p50 **11**。
  ⇒ 「Tier B」实际不是「衔接 Tier A 的过渡带」，而是**没进 Tier A 锚定的 9x6 电路堆放处**。
- **浅/小区间的余量**（V3 现在是唯一训练数据，这一格决定旧数据那批主流模板还在不在）：

| 区间 | 电路 | 行 |
|---|---|---|
| 深 ≤5 | 1,609（12.9%） | 22,186（**3.7%**） |
| 深 ≤6 | 2,299（18.5%） | 32,908（5.5%） |
| trans ≤104 | 3,780（30.3%） | 48,381（**8.1%**） |
| 深 ≤5 且 trans ≤104 | 1,457（11.7%） | 18,153（**3.0%**） |

**两个口径必须分开读**（沿用本仓库 recall 双口径习惯）：
按**电路数**浅/小还有 12.9%~30.3%，不算少；但按**行数**（= 训练样本量）只剩 3.7%~8.1%，
因为大形状每电路行数多（(9,6) 84 行/电路 vs (2,1) 4 行/电路）。

**风险（标「待复核」，不下定论）**：V2 全挤在「小 + 浅」，V3 全挤在「大 + 深 + 多输出」——
**方向恰好相反**。规格 §四表格说 Rust 46 模板里「其余 36 模板 … 浅（深 ≤6、trans ≤104）；
**旧数据已充分覆盖**」—— 这句话在 V2 还在训练集里时成立，
而 V3 单数据集方案（§四「训练数据角色」）把旧数据移出训练后**这个前提就不再成立**：
V3 只给这 36 个模板 8.1% 的行覆盖。**是否会把上一轮的「深/大 OOD」反转成「浅/小 OOD」，
只能靠训练后的分形状指标回答**（规格 §四量化验收 B 段的「旧数据固定 test 集」闸门正是为此设的，
v2wave42m4 同一切分仍在盘上）。此处只记测量，不预设结论。

### 18.6 弱驱动判据近于恒真（判据本身的问题，不是数据的问题）

规格 §四第 4 条的验收判据是「含『弱驱动中间门』（实例扇出 ≤2，或对地寄生 ≥ 该电路寄生中位 2×）的电路占比 ≥30%」。
实测 **100%**（两条子判据各自都 100%）。但门实例扇出分布（消费者数）为
`[0:8.8%, 1:23.9%, 2:32.7%, 3:4.4%, 4:9.4%, 5:2.3%, 6:6.0%, 7:1.0%, ≥8:11.3%]`，中位 2、均值 3.16
⇒ **扇出 ≤2 的门天然就占 65%**，任何一个网表都必然含「中间门扇出 ≤2」。
metadata 自评 `weak_drive.share = 1.0`、`fanout_median = 1.0` 是同一现象的另一面
（它的 1.0 是按「驱动者数」数的，恒为 1）。
⇒ **这一项判定为「过」，但它不能作为「Rust 候选 6.1% 负近似门现象已被覆盖」的证据**；
要验那件事得用 serve 侧同口径特征比对（列入待复核）。

### 18.7 metadata 自评 vs 本报告实测

**复现一致的部分**（说明双方没算错）：
- 逐形状 trans p50/p90/max：18 个形状里 17 个**完全一致**，只有 4x1 的 p90 差 3（自评 59 / 实测 56）。
  该形状 p50 与 max 都一致、只有 p90 差，**最可能是分位数口径之差**（本脚本用 `np.percentile`
  线性插值，交付方疑用最近秩；430 个样本时 p90 落在第 386/387 个之间）。**未定论，不下结论**。
- 相邻差四档：自评 `{<1%: 0.6727, 1-5%: 0.159, 5-20%: 0.1215, >20%: 0.0467}` vs 实测
  **67.3 / 15.9 / 12.2 / 4.7%** —— **逐格一致**。
- 组内 spread 中位：自评 0.1951 vs 实测 **0.195** —— 一致。
- Tier A 行占比：自评 0.3699 vs 实测 **0.370** —— 一致。
- `split_rows` 431,439 / 62,835 / 105,702 —— 一致（但**无读者**，训练自切划分，见 §17.8）。

**一处 2.5pp 差（标「待复核」）**：`groups_with_gt10pct_spread` 自评 **71.33%**、
本报告实测 **68.8%**（576/837）。相邻差四档逐格一致而这一项不一致，
怀疑是 spread 的分母口径之差 —— metadata 同时给了 `spread_median`（0.1951，与实测一致）
和 `spread_median_over_min`（0.2087），**若「>10% 的组占比」是按 /min 算的，就会比按 /median 算的多**。
未定论，不下结论。
⚠ 连带修正：§17.4 里「与交付方自评数字逐格一致」这个说法**应限于相邻差四档**；
spread 类占比有一处 2.5pp 差，不能笼统说逐格一致。

**缺报的部分**：深度分位（规格明确要求）、§18.2#10 那条指标、Tier B 内部的两条过渡带占比
（metadata 只给了全局 `row_weighted.band_140_190 = 0.0269`，而判据要的是 **Tier B 内**的，
实测 Tier B 内是 **0.0%**）。**用「最接近的可得数字」替代「判据要求的数字」会把未达标读成达标**，
这是本次验收里最需要记档的一处方法论问题。

### 18.8 随之而来的默认数据改动（18.1.0）

用户指示「之后更改默认使用数据，只用 v3」。改动与**它带来的两个静默陷阱**：

1. `config.py` `DATA_BATCHES` 默认 `'batch_v2_full,batch_v2_rest,batch_v2_m4'` → **`'v3_delivery'`**。
2. **`USE_V2` 不动**（仍是 `True`）。它标志的是「V2 版列结构 + `resolve_v2_batch_files` loader +
   `ranking_metrics` 的 mean 聚合口径 + `V2_STRUCT_MODE`」这一整套，**不是「用 V2 数据集」**；
   V3 走同一条路径。改它会连带改聚合口径与 STRUCT_MODE。已在 `config.py` 就地写清。
3. **`setup_exp.sh` 加「旧三批显式钉住」块**：`v2*|seed[0-9]*|struct*|waverich|cornerattn`
   → `export DATA_BATCHES="${DATA_BATCHES:-batch_v2_full,batch_v2_rest,batch_v2_m4}"`。
   **没有这一块，所有既有变体名会静默改吃 V3 数据而名字一个字母不变** ——
   跑得完、退出码 0、日志正常，正是本仓库最怕的静默失败。
   最要命的是 `v2wave42m4`：规格 §四量化验收 B 段把它当**跨分布通用闸门**，
   它若变成 V3 数据，**闸门就自己验自己了**。已逐名验证路由（v3nowave42→v3_delivery，
   v2nowave42m4/v2wave42m4/seed456/structrich/waverich/cornerattn/structlogic2468→旧三批）。
4. **6 个 diag 脚本各自硬编码了旧默认串，故意不动**：
   ...
   ⇒ 它们的取数仍走旧三批，**与 `config.DATA_BATCHES` 不再一致，这是刻意的**，已在 `setup_exp.sh` 就地记档。
5. `data/batch_v2_*` **不删**（用户口径是「不用了」，不是「删掉」）；它同时还是
   §四量化验收 B 段的闸门数据。
...
| 脚本 | 作用 |
|---|---|
| `scripts/diag/_t_v3_requirement_check.py` | 本节全部数字。深度按规格口径（**X_ 宏级最长链**）自算；逐形状配额/规模档/深度档判定；Tier A/B 画像与联合分布；I/O 分桶；弱驱动（**扇出按消费者算**）；组内延迟差三条硬指标；metadata 自评对照 |
| `scripts/diag/_t_v3_group_spread.py` | §18.2#11/#12 的独立复现（§17.4 已记），含 V2 三批对照与逐形状分解 |

只读 `data/` 与仓库代码，**不改文件、不跑训练**。

**本报告自查记录（写下来防复踩）**：
① 初版把 `len(prod[net])`（生产者数）当成扇出 ⇒ 恒为 1、判据必然 100%。已改为按**消费者数**算，
   §18.6 的数字是修正后的；metadata 的 `fanout_median=1.0` **疑似**同一类口径问题（恒为 1 只能是
   「驱动者数」而非「消费者数」），但交付方代码未公开，**未定论** —— 值得回问生成方。
② 深度必须自算 —— 交付方没给，不能拿 trans 分位代替「深度分位」交差（§18.7 末）。

---

## 十九、V3 缺陷清单（2026-09-20）→ 详见 `docs/V3_ISSUES.md`

§十七/§十八回答的是「V3 能不能用、满不满足当初的要求」；本节只做**指引**：
V3 已知缺陷的**完整清单、按影响排序、逐条修法与责任方**另立一册
**`docs/V3_ISSUES.md`**（同一批只读实测的证据，脚本见该文档 §2）。
两册数字**同源同口径**，引用时以 `V3_ISSUES.md` 为准；本节的 22 条验收结论不重复。

⚠ **`V3_ISSUES.md` 的定位在 2026-09-20 改过一次**：前提改为「**生成方重新生成全量数据**」，
该册因此**只保留会影响生成的部分**，等同于交给生成方的**重出规格**（必改清单在该册 §4）。
判入规则两条：① 生成方要改数据的；② **我方改代码就能跑通、但跑通后数据仍不达标或
信息变少的** —— 后者标记 **🔶 可绕·不许绕**，共 5 条（P0-3 / P1-2 / P1-3 / P2-6 / P2-7）。
**这 5 条原先写在各自「怎么修」里是"我方动作"，现全部升级为对生成方的要求。**
纯我方侧（脚本、本地流程坑、已核验无问题项）移入该册 §5 非必改附录。

⚠ **该册同时新增了「★ V3 做对的地方（重出时必须原样保留）」一节，排在正文最前**：
把本节 §18.2 里 **14 条 ✅**（22 条 = 14 ✅ / 7 ❌ / 1 ⚠）与 §18.3 的**三条根因已解决**
写成对生成方的**"不许改坏"要求**
（★1 三条根因、★2 逐条达标项、★3 数据完整性、★4 两项名义达标要按去重口径复算、
★5 宏粒度偏大但**不作为要求**）。**只给缺陷清单、不给"必须保留"清单，重出时会把
已解决的三条根因又改回去** —— 这是本次补该节的原因。

**三个必须修改项（🔴，摘要）**：

| # | 缺陷 | 一句话 |
|---|---|---|
| P0-1 | **组内重复候选**（V3 引入的回归） | **55.7%** 电路在自己的 expr 组内有**设计级孪生**（静态全签名 + 逐弧 DELAY 全同，60 簇抽样验证逐字节相同）；**40.3%** 是纯冗余副本（**37.2% 是下界** —— 含 `transistor_count` 的签名会放过 389 个假差异）；平均有效组大小 14.88→**9.35**。V2 三批同口径只有 0.2-4.7% ⇒ 不是这条数据线固有的 |
| P0-2 | **形状配额** | 名义口径 6/18 不合格 → **可分设计口径 17/18** 不合格；**(1,1) 两种口径都是 0**（结构性 OOD） |
| P0-3 | **`struct_prior` 三列中两列死亡** | V3 `#SC_AND == 0` **100.0%**（12,455/12,455）、`#SC_INV_WIRE == 0` **89.6%**；两列同时为 0 的占 **89.6%** ⇒ 三列结构先验实际只剩 `transistor_count` 一维在动 |

**对既有结论的直接影响（读 §十七/§十八 时必须带上的口径）**：

1. **§17.4/§18.2#11 的相邻差四档要按两档读**。原始口径 `<1% = 67.3%` 里
   **55.7pp 是「恰好 0」**（组内重复的直接后果）；**去重后**为 `<1% = 45.6%`（恰好 0 只占 26.3%）、
   `1-5% = 26.4%`、`>20% = 7.8%` ⇒ 规格判据 **②④ 在去重后是达标的**，
   不去重会被读成两条都不达标。**这是 P0-1 造成的一处误判，必须改口径。**
2. **§18.2#2 的形状配额**同理：引用「交付 N 个形状」时必须注明是名义口径还是可分设计口径。
3. **§18.7 的「缺报深度分位」** 与 §18.6 的扇出口径问题，在 `V3_ISSUES.md` 里升格为
   P2-2（🟠 应当修改，成本几行）与 P2-6（🟠 应当修改 + 🔶 可绕·不许绕 ——
   **判据近于恒真，作废它不等于原目的达成**，须生成方给覆盖证据或换一条有鉴别力的判据）。
4. **新增一条利好**（`V3_ISSUES.md` P2-8#1）：`transistor_wave_json` 的 `gate` 键
   vs netlist X_ 实例名，**命中率 100.00%**（3,246,051 / 3,246,051，等距抽样）
   ⇒ 波形特征能贴上，不存在「连接键名域不匹配导致整块静默变 0」。**不必再查。**
5. **P1-4 已定论、P1-1 已定性并拆成两条**（A 组三项，2026-09-20）—— 见本节 **§二十**：
   `ids_avg` 条目级 54.7% → **节点级/特征级 39.5%**（wave 覆盖率实测 **100.00%** ⇒ ②==④），
   **不是死维，是对生成方的这条要求已撤销**；`transistor_count` 不自洽经 V2 两批
   **全部自洽**对照 ⇒ **P1-1 是 V3 引入的回归**；归因（§20.4）后又拆出**一条独立要求** ——
   368/371 组是「列噪点」，**3 组 20 电路是「交付静态记录缺维度」**（六列全同、延迟不同）
   ⇒ 模型输入层面的**同一输入、两个标签**。

⚠ **口径纪律（本次新增，写入后所有 V3 读数适用）**：
① 名义口径 vs **可分设计口径**；② **行**口径 vs **电路**口径；③ 抽样 vs 全量 ——
三者混用会得出相反结论（`V3_ISSUES.md` §0）。凡分片数据**禁止用 `head()` 抽样**：
分片按电路顺序写，实测同一列头尾能差 50 个百分点（同文档 §3 自查记录①）。

## 二十、A 组三项收口（2026-09-20）：P1-4 定论 / P1-1 定性 / P2-3 提问清单

本节是 `V3_ISSUES.md` 重出规格定稿后**只剩我方做得了**的三件事（A1/A2/A3）的实测记录。
三件都不产出新数据、不影响任何一次训练的输入，只改**我方对 V3 的判断**与**给生成方的提问**。

### 20.1 A1 = P1-4 定论：`ids_avg` **不是死维，是与规模耦合的稀疏维**

**为什么必须补测。** 已测的是**条目级**（V3 part01 全量 49.0% / part16 57.0% / 等距 31 片
54.7%），但 `data_loader.py:643-665` **不是**按条目喂特征，而是**按 gate 取该门全部晶体管
的均值**（`g = str(td['gate']).lower()` → `tw_feat[i, fi] = arr.mean()`）。⇒
一个门的 `ids_avg` **只在它全部晶体管都为 0 时才为 0**，节点级零占比必然 ≤ 条目级。
原文书 L510 明写「在测出节点级零占比、并与 V2 结点级对照之前，不把它写成「特征死了」」。

**脚本**：`scripts/diag/_t_v3_idsavg_node.py`（只读，聚合口径逐字照抄 loader，含
`if v is not None` 与 `if not vals: continue` 两个分支）。抽样为**整片等距**（每片 400 行）。

**★ 四个口径（不可互读）**：

| 口径 | 定义 | 分母 |
|---|---|---|
| ① 条目级 | 全部晶体管条目里 `ids_avg == 0` 的比例 | 晶体管条目数 |
| ② 节点级 | 一个有 wave 的门，其全部晶体管 `ids_avg` **均值 == 0** 的比例 | 有 wave 的门数 |
| ③ 行级 | 一行（circuit / pin / dir / output / corner）里**所有门**都为 0 的比例 | 行数 |
| ④ 特征级 | 非零节点数 / **电路总门数** —— **这才是模型看到的那一维** | Σ 电路门数 |

② == ④ **仅当** wave 覆盖电路全部门。本次实测**覆盖率 = 100.00%**（12,408/12,408 行、
682,686/682,686 槽位，三个数据集全为 100%）⇒ **② == ④**，39.5% 可直接当特征零占比引用。

| 数据集 | 有效行 | ① 条目级 | ② 节点级 | ③ 行级全零 | ④ 特征级 |
|---|---|---|---|---|---|
| **V3 v3_delivery** | 12,408 | **54.7%** | **39.5%** | **0.0%** | **39.5%** |
| V2 batch_v2_full | 8,670 | 27.2% | **5.1%** | — | 5.1% |
| V2 batch_v2_m4 | 1,800 | 46.9% | **32.7%** | — | 32.7% |

**自校验通过**：本探针复算的 V3 条目级 = **54.7%**，与 `V3_ISSUES.md` P1-4 已有的 54.7%
**逐位相同** ⇒ 没跑偏。

**定论（三条）**：

1. **不是死维**。零 = **该门在这个向量下不翻转**，不是数据没填。
   节点级非零取值分布（n = 413,270）：min 2.3e-4 / p10 2.9e-3 / **median 0.125** /
   mean 0.375 / p90 0.865 / max 109 —— 中位数 0.125 是个实打实的量级，不是浮点噪声。
2. **是稀疏维，且稀疏度与电路规模耦合**：V2-full **5.1%** → V2-m4 **32.7%** → V3 **39.5%**。
   电路越大（门越多），「整门全部晶体管都不翻转」的比例越高 —— 这与行级全零
   **0.0%**（没有任何一行是整片不翻转的）互相印证。
3. **不写进重出要求**。原先拟向生成方提的「让 `ids_avg` 有真实值 / 把 55% 的 0 修掉」
   **撤销** —— 要求生成方去改一个物理上就不是恒非零的量，只会诱发填假值。
   `USE_TRANSISTOR_WAVE` 的默认值（开）**保持不动**。

**连带口径修正**：`V3_ISSUES.md` §1 与本节 §十九 里引用的「55% 条目为 0」
**只能作条目级引用**；特征层面的正确数字是 **39.5%**。

**新观察 🟡（未定论，已并入 P2-3 的 Q5 回问生成方）**：`vds_swing` 的节点级零占比
V3 **21.9%** vs V2-full **0.1%** / V2-m4 **0.9%**（高一个数量级），而非零中位数只是略低
（0.05 vs 0.184 / 0.065）。**「没翻转」解释不了** —— 一个不翻转的门 `ids_avg` 与
`vds_swing` 应当**同时**为 0，但 `vds_swing` 的零占比只有 `ids_avg` 的一半。

### 20.2 A2 = P1-1 定性：`transistor_count` 不自洽是 **V3 引入的回归**，不是老账

**脚本**：`scripts/check_transistor_count.py`（只读）。网表列口径同 `data_loader.py:80-82`
（优先 `gate_level_netlist_std`，loader 会 drop 原始 `gate_level_netlist`），
⇒ 本检查看到的就是**模型看到的那张网表**。

| 数据集 | 不自洽 netlist | 涉及电路 | 同 netlist 极差 |
|---|---|---|---|
| **V3 v3_delivery** | **371 / 5,900 = 6.3%** | **1,847 / 12,455 = 14.8%** | 中位 6 / p90 16 / max 32 |
| V2 batch_v2_full | **0 / 8,679** | **0 / 8,679** | — |
| V2 batch_v2_m4 | **0 / 1,800** | **0 / 1,800** | — |

**★ 结论**：V3 的 371/6.3%/14.8%/中位 6 与 `V3_ISSUES.md` P1-1 已记的数字**逐位复现**；
**新增的是 V2 两批全部自洽** ⇒ **P1-1 是 V3 引入的回归**，与 P0-1（组内重复）同类。
`V3_ISSUES.md` §1 与 P1-1 条原先的分类「长期存在」**应更正为「V3 新增回归」**（已改）。

**影响**：该列既是输入特征（`data_loader.py:322`）又是 `struct_prior[0]`（`:716`），
而 `struct_encoder` **无任何输入归一化**（`src/model.py:50-54`）⇒ 等于给模型注入
「两张同样的图、同样的延迟，尺寸却不同」的噪声，直接对抗排序目标。

**接线（`setup_exp.sh`）**：V3 臂（`v3wave*|v3nowave*`）**每次开跑前**调用一次，
**只告警不阻断**（`|| true`，脚本自身退出码恒 0）。理由写在脚本文件头：当前 V3 **确实**
命中 14.8%，写成硬失败会把每一次 V3 运行都挡在门外，而它挡下的是**已经知道的事** ——
检查的价值在「下一版别再犯」与「将来某次运行突然变好/变坏要看得见」，不在拦住今天这一次。
生成方给出该列定义、使其成为电路内容的函数之后，本检查应**恒静默**；到那时才考虑 `--strict`。

### 20.3 A3 = P2-3 提问清单（要**计算式**，不要数字）

清单已成文写在 `V3_ISSUES.md` §P2-3 末尾「附：向生成方的提问清单（**要计算式，不要数字**，
可直接转发）」，共 **Q1–Q5**：

| # | 问的是哪条 | 要什么 |
|---|---|---|
| Q1 | P2-3 `groups_with_gt10pct_spread` | 分母是 `/min` 还是 `/median`（我方复算 71.33% vs 生成方报 68.8%，两种口径各对一种） |
| Q2 | P2-3 `weak_drive.fanout_median = 1.0` | 「扇出」的定义 —— 实测 1.0 更像**扇入** |
| Q3 | P2-3 4x1 `trans_p90` 59 vs 56 | 分位取法（插值 vs nearest-rank） |
| Q4 | **P1-4（新）** | `ids_avg == 0` 的**成因分类**（不翻转 / 未收敛 / 缺测），要**判据**不要比例 |
| Q5 | **P1-4（新）** | `vds_swing` 零占比高一个数量级的成因；以及它与 `ids_avg` 不同步是否为物理预期 |

**为什么只问计算式**：以上五条我方都能拿到数、也都能复算，**差的是口径**。
要数字只会再得到一层无法互相印证的结果；要到计算式才能把「差 2.5 个百分点」「差 3」
这类残差一次性归零或定性为真实分歧。

### 20.4 A2 归因（本轮追加）：`transistor_count` **不是一个病，是两个**

§20.2 只说了「不自洽」，没说**是哪种**。归因探针 `scripts/diag/_t_v3_tc_cause.py`（只读，全量）
把 371 个不自洽 netlist 按**组内延迟签名**（该电路全部弧的 `DELAY` 排序后逐值比较，
签名相同 = 同一批仿真结果）分解：

| 组内延迟签名 | 组数 | 含义 |
|---|---|---|
| **逐值全同** | **368 / 371** | 同一批仿真结果却给了不同 `tc` ⇒ 错在**这一列**（噪点） |
| **不同** | **3 / 371**（20 电路） | 静态列**全同**、延迟不同 ⇒ 错在**「文本不是单射」**，交付记录缺维度 |

**排除了两个更省事的解释**：① 不是取整 —— 极差 == 1 的 **0** 个（2 占 4.6% / 4 占 22.9% /
≥5 占 **72.5%**）；② 不是宏集不同 —— 371 组的 `cell_types_json` **组内全同**（0/371 不同），
`parasitic_caps_json` / `pin_loads_json` 亦各 **0** 组不同。交付 netlist **只写宏名不带宏体**
（**0 / 12,455** 行含 `.SUBCKT SC_`），宏体在 `sc_expansion.json`（1,049 宏、**一名一体**）
⇒ 按「文本 + 宏表」反推 ASAP7 单元实例数**只能得一个值**，推不出两个 `tc`。

⇒ 要求**拆成两条**（已写进 `V3_ISSUES.md` P1-1 与 §4 第 3 条）：
① `transistor_count` 必须是电路内容的函数（368 组证明当前不是）；
② **交付的静态记录必须唯一确定电路**（3 组 20 电路证明当前不是）—— **② 更重要**：
它是模型输入层面的「同一输入、两个标签」（那 20 个电路拿到逐字节相同的图、对应不同延迟），
是训练侧**不可约**噪声；① 只污染 `struct_prior[0]` 与那一个标量特征。

⚠ **这一列两个方向都有**（那 20 个电路的 `tc` 差异与延迟差异**同向** ⇒ 在 3 组里 `tc` 是对的）
⇒ 它**既不能当特征、也不能单独当去重判据**。连带：P0-1 的去重键（不含 `tc`）会把那
**20 个（0.16%）真不同电路**合并 —— 不改 P0-1 的结论，引用时带上这句边界即可。

**连带修正**：`scripts/check_transistor_count.py` 现在多打一行「该文本被几个电路共用」，
让下次一眼分清「列噪点」与「文本非单射」；后者的完整判据要看时序弧，指向上面这个探针。

## 二十一、V3 交付栈首轮部署判读（2026-09-22，18.5.1）：严格@3 退步 15pp，宽松@3 持平

本节兑现 PROJECT_LOG §18.5.0 结尾那句「部署是否变好只能由 Rust shadow 回答」。
数据 = 2026-09-22 23:00:21 → 23:02:26 那趟 shadow：8000 挂
`~/project-107-v3nowave42/outputs/midpoint_ep200.pt`（sha1 `51bd323382ecb9b7`，
实测 `in_features=45` = 纯拓扑 ⇒ serve **不带 env**）+
`scaler.pkl`（sha1 `6b7322d6dc8bd56e`）；**复用旧缓存**（`~/NetlistOpt/temp_sim_cache/`
134,129 → 134,129，**零增量 = 零 Xyce 重算**，全程 1m48s）。

### 21.1 先证两臂可比（四项前置，全部通过）

| 项 | 证据 |
|---|---|
| **候选集** | 与基线归档树 `~/shadow_archive/midpoint_ep250_20260917_214841` **逐项相同**：714 集（≥4 候选）/ 成功 **5390** / 失败 **8** / 重复网表集 **31** / hi-spread **591** / 独立池 **161** |
| **输入污染** | 三项检查**全 0**（分组内 eval 去重丢弃 / 同电路 eval_idx 跨分组重复 / 同一 eval_idx 真值冲突） |
| **排序量纲** | 量纲混合守卫 **0/714**、兜底窗口 **0/714**（全走秩聚合） |
| **分析器** | 基线树用**同一版**（`_shadow_analyze.py=d8977633773a7f59`）`--root` 重算 = 严格k3 **90.8** / 宽松k3 **97.9** / 遗憾 **5.93**（中位 2.13）/ 两阶段 **0.63**，②③④ = **80.1 / 81.5 / 76.7** ⇒ 与 §6.7 权威数**逐位相同**、无漂移 |
| **排除的旁证** | 「预测有重复值」57 vs 56 集、「第3==第4」5 vs 8 ⇒ **并列不是 −15pp 的成因**；serve 侧 8 个文件 blob 两臂逐行相同（特征码未换） |

### 21.2 ① 714 集 batch（部署真口径）

| 指标 | 基线 `v2nowave42m4` ep250 | **V3 `v3nowave42` ep200** | Δ |
|---|---|---|---|
| 严格@2 | 71.8% | 68.8% | −3.0 |
| **严格@3** | **90.8%** | **75.8%** | **−15.0** ❌ |
| 宽松@2 | 83.3% | **89.4%** | **+6.1** |
...
### 21.3 四口径 + `level2/DEPTH_MIX` 反解

| 口径 | 单位数 | 严格k3 | 宽松k3 | 选择遗憾 |
|---|---|---|---|---|
| ① 全部批（等权） | 714 | 90.8 → **75.8**（−15.0） | 97.9 → 97.5 | 5.93 → **5.37** |
| ② 按候选池去重 | 161 | 80.1 → 73.3（−6.8） | 96.3 → 95.7 | 3.75 → **3.03** |
| ③ 按电路宏平均 | 18 | 81.5 → 72.2（−9.3） | 96.1 → **98.9** | 5.34 → **4.93** |
| ④ 排除 `level2/DEPTH_MIX` | 176 | 76.7 → 73.3（−3.4） | 91.5 → **96.6** | 3.03 → **1.73** |

**形状一致**：**严格 k 四个口径全部变差；选择遗憾四个口径全部变好**；宽松 k2 在 ①③④ 变好（+1.7~+9.4pp）。

**反解（推导值，非直接报数，可核）**：① 与 ④ 都是**逐集等权均值**，且 538 + 176 = 714 ⇒
`D = (714·① − 176·④) / 538`，得 **`level2/DEPTH_MIX` 单电路严格@3：基线 95.4% → V3 76.6%（−18.8pp）**，
其余 176 集只差 **−3.4pp**。⇒ **退步大部分压在 DEPTH_MIX 一家**（它在 ① 里占 75.4% 权重）。

### 21.4 结论：**圈得稳，挑不出第一**

- **圈得住**：宽松@2 **+6.1pp**、选择遗憾**四口径全部变好**、两阶段遗憾与捕获率持平、
  Spearman 中位 0.786 vs 0.714 —— 「对的那一小撮」仍在。
- **挑不出**：严格 k **四个口径全部变差**（k3 −3.4 ~ −15.0pp），**hi-spread 591 集 −17.2pp**
  —— 恰恰在最该排对的地方最差。
- 一个不能省的细节：V3 的改善主要来自**尾部**（均值遗憾更好，而**中位反而 2.57 > 2.13**）。
- **两臂主判据都仍 ❌**（选择遗憾都 >5%、集合版 recall@3 都 <90%）⇒ GNN 仍是**启发式预排序**。
- **交付决定（用户 2026-09-22）**：**维持 `midpoint_ep250`，不切换**。

### 21.5 两条限制（引用本节数字时必须一起带）

**(a) ⚠ 特征表没有变（18.5.2 修正）—— 两臂 `in_features` 实测都是 45。**
恢复 serve 前按 §6.2 Step 1 探针实测：交付基线 `midpoint_ep250.pt` = **45**、V3 `midpoint_ep200.pt` = **45**
⇒ **输入布局同一、都「不带 env」** ⇒ 本趟比较**不掺特征表变更**，两臂差异只在**数据**
（`v3_delivery` vs `batch_v2_*`）与 ckpt ⇒ **−15pp 可归因到「V3 这一版训练出的模型」**。
⚠ **本节 18.5.1 初稿此处写错了**：曾按 I9 旧记写「基线 in_dim **14**、V3 **45**（= 14 静态 + 31 弧特征）、
归因分不开」—— 数字错（实测皆 45），且把启动日志 `1 static + 31 arcs` 的「31」误读成**特征数**
（它是 **31 个 `timing_arcs_partNN` 分片文件**，18.0.0 原记即如此）。**该限制整条作废。**
⇒ 被它挡住的推论现在成立：**V3 数据的这一轮训练在部署口径上确实退步了**。
仍未排除的是**单 ckpt 采样**（ep200 vs ep250 不同 epoch；跨 epoch 复测属 I15 同类待办）。

**(b) 106 集 window 口径（已废，仅存档）—— V3 该口径下守卫 ❌，且存档行复现不上。**
V3：严格k3 44.3 / 宽松k3 66.0 / 遗憾 **6.99**；同口径**重算**基线：47.2 / 70.8 / **9.59**
（方向与 ① 一致：严格略差、遗憾更好）。但 **V3 有 12/106 集量纲混合守卫 ❌**
（集内混「秩」与「原始延迟」⇒ 排序无意义）⇒ **该口径不得作判读依据**。
另：§6.7 存的 106 集基线行（遗憾 **11.13** / 严格k2 24.5 / 严格k3 **34.9**）与今天在
**同一棵树**上重算（9.59 / 36.8 / 47.2）对不上 —— **已记 I21 待查**（batch 口径那边逐位复现
⇒ **不是分析器漂移**；怀疑存档行出自 09-15/09-16 另一棵树）。

**🆕 连带发现（I20）**：`_shadow_analyze.py` 的「运行配置戳」对 `--root` 归档树而言，
`serve ckpt`/`scaler`/`端口`/`环境`/`repo`/`脚本 sha1` 读的是**当下在跑的 serve 与当前工作区**，
只有 `分析根目录`/`CSV 快照` 来自被分析的那棵树 —— 本节两次戳（基线归档 vs 本轮活树）
这几行**逐字相同**即为实证。⇒ 拿戳给归档树溯源会**把归档树误标成本轮模型**（I15/74.4 同类错标的又一个入口）。

## 二十二、以 current 为锚的 GNN 判据质量（18.6.0 落地 / 18.6.1 核心口径 + 配对前缀修正）

本节兑现用户 2026-09-23 的口径：**「在评测处新加入指标，不看所有比 current 大的，只看排名第一是否实际是负，这个指标作为核心判据」**。`tl_opt.rs` **零改动**：观测走已存在的钩子 `rank_window(&current.module, &mods, eval_idx, total_iters, window_try_idx)`（签名里本来就有 `current`，默认实现 `Ok(None)`），shadow 覆写**恒返回 `Ok(None)`** —— 返回 `Some` 会让调用方重排 `prepared` 或整窗 `continue`，返回 `Err` 会经 `?` 中止整个电路。**仿真继续掌判据，GNN 在运行中零决策权。**

### 22.1 为什么单独加这条

本文件其余全部指标都把 GNN 放在**候选集内部**评分（「这批里谁最快」），参照系里**没有 current 这个角色**。而部署上贪心的实际判据是 `tl_opt.rs` Pass 3：按生成序逐个 `evaluate`，**撞上第一个 `delta < 0.0` 就 `accept` 并 `break`**（`tl_opt.rs:1156-1191`）⇒ GNN 掌判据时的失败模式是「把一个**其实不如 current** 的候选判成比 current 好」，于是它会建议先试那个候选、试探成本照付。

### 22.2 两把尺不可互读（数据来源与量纲纪律）

- 新落盘 `gnn_current.csv`（每窗一行组）：`CURRENT` 行 + 各候选行；`rank` = 「**`[current]` + 本窗口全部候选**」放进**同一批** `/rank` 得到的 competition rank（1-based、并列取平均秩）。
- `gnn_shadow.csv` 的 `gnn_pred` 是「**纯候选批**」的同类秩 ⇒ 同一个候选在两个文件里的秩**不同**，**不可互换、不可跨文件比较**；本节任何数字不得与主口径的 `Sp / recall@k / 选择遗憾` 对读（`per_window_metrics` 对 `gnn_pred` 用 `argsort` 的序数秩、**无并列修正**，与 serve 给的真实平均秩不是同一个约定）。
- 候选**真值只能**从 `gnn_shadow.csv` 按 `eval_idx` join（本文件候选行 `true_avg` 一律 `NA`，刻意如此，防有人误读本文件自带真值）。

### 22.3 核心口径（18.6.1）：只看首推那一个

- **首推** = 该窗候选里**按秩**最小者（并列取同秩**整组**）。
- **严格** = 首推组**全部** `t1 < t_cur`（与 `delta < 0` 同调）；**宽松** = 全部 `t1 <= t_cur`（不劣）。并列组内真假不一 ⇒ 按**保守**判错并单列 `t1_mix`。
- **首推无真值**（落在未评估后缀 / 该次 eval 失败）⇒ 剔出分母、单列 `n_t1_nodata` —— 这条不能省：**部署上贪心先试的正是首推那个**。
- **三桶恒等式**（互斥且穷尽）：`首推 current` ＋ `首推候选无真值` ＋ `进分母` = 合格集。⚠ **镜像桶不能用 `S == 0` 定义**：`S` 只数「秩与真值都齐」的候选，而首推只看秩 —— 首推那个候选真值缺失时 `S` 仍可为 0 ⇒ 同窗双计、恒等式破；故改为按**秩**判（`min(rank候选) < rank(current)`）。
- **镜像漏推率** = 「GNN 首推 current」的窗里其实**存在更优候选**的比例 ⇒ 防「永远说 current 最好」把首推率衬托得好看（是**下界**：未评估后缀里的更优候选无从知道）。
- 旧的逐候选指标（误报率 / 漏报率 / top-1 命中 / 秩相关）**降级为参考口径**（分母是候选不是窗，核心结论一律引首推率）；四口径表与单/多输出分层表各多出 `★推严 / ★推宽` 两列。

### 22.4 ⚠ 18.6.0 首跑读数全部作废（含那组 99.71%）—— 不是数据错，是校验口径错

1. 18.6.0 的配对校验要求「每个候选行的推断 `eval_idx` 都必须在 `gnn_shadow.csv` 里存在」，首跑实测 **134 个窗口**不满足 ⇒ 报告打 ❌、该窗被剔。
2. 为什么不满足是**预期结构**：Pass 3 **首个 `accept` 即 `break`** ⇒ 排在其后的候选**永不评估、永无 `eval_idx` 行**；而本文件候选行出自 `mods`，`mods` 由 `prepared` 现取（`tl_opt.rs:1024-1026`）⇒ `present` 只能落在**前缀**上。
3. 更要紧的是这条校验**恰好筛掉了 GNN 可能说对的那批窗**：真有改进 ⇒ 贪心 evaluate 到它就 accept+break ⇒ 尾部未评估 ⇒ 配对失败 ⇒ **整窗被剔**；留下的几乎只有「全窗无改进」⇒ 误报率机械地被推到 ~100%。**首跑那组 99.71% 就是这么来的，不是模型差 —— 不得引用**（同批其余数字一并作废）。
4. 18.6.1 改法：`present` 的 `pos` 集必须恰为 `{0..k-1}`（有洞 / 零个 = **真错位** ⇒ `n_ev_missing`）＋ 该窗 shadow 行集恰为 `{base+1..base+k}`（`n_mismatch`）＋ 逐行 `tc` 相符 ＋ 窗内 `eval` 不重复；缺的后缀单列 `n_uneval`，并另报其中「**秩高于 current**」的项数 `n_uneval_above`（它们无真值 ⇒ 进不了 `S` ⇒ **`S` 被低估这么多，必须可见**）。
5. 判决书 = 重跑时诊断行 `推断位置非前缀: 0`（该行标签已由「eval_idx 不存在」改名，说明缺的是**位置**不是**行**）。

### 22.5 读法边界（别让它变成下一个 I15/I20）

- **必要不充分代理**：本节真值是 `avg_delay`（全输出口均值），贪心吃的是 `affected_delay`（本窗 `affected_outputs` 子集）⇒ 两者**只在「受影响口 = 全输出口」时恒等** ⇒ **单/多输出分层是必需项、不是可选项**（多输出占候选集 52~57%，含权重最大的 `ADD4_OVF`/`COMP4`）。被本节判「误报」的候选仍可能在 `affected_outputs` 上真优于 current（贪心会**正当接受**它）。逐口真值版、取 trace `verdict` 与贪心实际接受对账 = **后续项，不在本方案**。
- **并列不对称**：`rank(候选) < rank(current)` 用严格小于 ⇒ 并列只判「漏报」不判「误报」，故并列计数单列。
- **首轮不设判据线**（用户拍板），只报数。
- 戳里 `_shadow_analyze.py` 的 `_sha16` 本次**必然变**（改了分析器）—— 这是**预期**不是漂移（`ENV_KEYS` 未变、未加新环境变量），别把它变成 I15/I20 那类「拿戳误判身份」的新入口。

### 22.6 读数

🟡 **待服务器重跑**（18.6.1 落地后的首个读数）。要填的格：首推真负率严格 / 宽松、分母与三桶缺口、池化 Σ真负 / Σ窗、镜像漏推率、并列数（含 `t1_mix`）、诊断行的 `推断位置非前缀` / `结构性未评估` / 其「秩高于 current」子数。命令与判读顺序见 OPERATIONS §6.8。

## 二十三、全组仿真最好者胜 + 栈式回溯 + 预算闸门与两道保险（17.5.0 / 18.7.0 / 17.6.0 / 18.7.1）

本节记的是一条**弧线**：先换掉贪心的搜索语义（内层 17.5.0 + 外 18.7.0 的核心口径与驱动变体），再给它套上预算与两道保险（内层 17.6.0 + 外 18.7.1）。用户 2026-09-23 的口径原文：「一个循环中，对一个窗口中所有的衍生电路跑仿真，在其中选择仿真成绩最高的作为下一个循环的 current；如果没有比 current 好的，就回退至上一个循环，选择第二名……以此类推」。⚠ 本节同时**补上前两轮漏记的档**（17.5.0/18.7.0 此前只在版本表里留了行、没有本节这样的正文）。

### 23.1 旧贪心的两个结构性缺陷（为什么要换）

1. **尾部候选永不评估**：Pass 3 按生成序逐个 `evaluate`，**撞上第一个 `delta < 0` 就 `accept` 并 `break`**（`tl_opt.rs:1156-1191`）⇒ 该窗排在后面的候选**一次仿真都没跑**。这既浪费候选预算，也让「GNN 的排序对不对」在尾部**无从检验**（无真值 ⇒ 进不了任何分母）。
2. **判据是窗口局部的**：接受看 `affected_outputs` 上的 delta（`combined_score` 里晶体管数是死代码），与「全电路最优」不是同一件事。

⇒ 新模式的**全部意义**在于**每组候选都有真仿真真值**。这同时解释了它与 GNN-only 为什么互斥：GNN-only 是搜索期零仿真，同时设置时按 GNN_ONLY 走并打一次告警。

### 23.2 新搜索语义（`TL_BESTFIRST=1`，默认关 ⇒ 不设时旧贪心行为逐字不变）

- 每个 `(轮, 窗)` = 一个**组**：组内**全部**候选逐个真仿真（不再提前 break）；**成绩最高者**（不是生成序第一个）成为下一轮的 `current`。
- 组内无改进 ⇒ **回退**到更早那一轮留下的窗组、**改选第 2 名**；该层用尽再退一层。计数落在 DONE 行的 `backtracks` / `max_depth`。
- `TL_MAX_STEPS` = 轮数预算（与 batch 的 `max_iters` 同口径，显式写进驱动 `MODE_ENV` 是为了进运行身份戳）；`XYCE_CACHE_DIR` 可配（17.5.0 起）⇒ 新模式写**新缓存目录**，不与旧全量搅在一起。
- 落盘形状不变：`gnn_current.csv`（`rank_window` 观测，1+n 行/组）＋ `gnn_shadow.csv`（逐候选真值）。

### 23.3 核心指标 R1/R2（用户口径；18.7.0 落地）

打印块 = 分析器 `★ 机会集口径 R1/R2（18.7.0；分母 = 真值上存在机会的组，分子 = 其中 GNN 把 current 排第 1 的组）`，列 = `组数 / D3 / D4 / N1 / N2 / R1严格 / R2宽松`，四行分层 = 全部组（`min_cands=1`）/ 其中 ≥`min_cands` 候选 / 单输出层（`nout=1`）/ 多输出层（`nout≥2`）。

- **分母 = 机会集**：`D3` = 组内真值上**存在比 current 更好**的候选的组数；`D4` = 存在**不劣**（更好或相同）者的组数（`D4−D3` = 只有真并列的组）。
- **分子 = 错说 current 最好**：`N1`/`N2` = 其中 GNN（同一批 competition rank）**仍把 current 排第 1** 的组数。「排第 1」= 没有候选秩**严格小于** current（同秩算并列第 1，与核心口径的镜像判据同一个约定）。
- ⇒ **R1 = N1/D3、R2 = N2/D4，越大越坏**（它数的是「本来有机可乘、GNN 却仍首推 current」的比例；分母为 0 的组单列、不入比值）。
- ⚠ **与 18.6.1 的「首推真负率」不是同一把尺**：那个问「首推候选是否真比 current 好」，这个问「GNN 有没有把 current 顶到第一」；分母构造也不同（机会集 vs 合格集）⇒ **两者不可互读、不可互相验证**。
- ⚠ 展示必须**同时给严格（R1）与宽松（R2）**两条；本块另有**自带的更严前置校验**（配对/覆盖 `pos` 集必须为 `{0..n-1}` **全集**、`current` 秩或真值不缺、候选秩与真值不缺）——四项全 0 才 ✅，否则诊断行打 ❌ 并写明「该树不是全组仿真的 ⇒ R1/R2 的分母只是下界」。**这四项是本节数字的判决书**：非 0 就别读 R1/R2。

### 23.4 两道保险（17.6.0）

- **单次仿真墙钟 `XYCE_TIMEOUT_S`**：加在**模板** `src/process_template/xyce.sh`（crate 从不直接 spawn Xyce —— 它渲染模板、再 `bash <script>`，Xyce 是脚本的子进程 ⇒ 套 `timeout` 的粒度正好是「一次仿真 = 一个 deck」）。超时以**约定的退出码 42** 收场（`simulation.rs::XYCE_TIMEOUT_EXIT_CODE`，模板与 Rust 的唯一约定点）；Rust 侧在 `if !output.status.success()` 里**最先**判 42 ⇒ **跳过 NOOP 重试**、且**超时的 deck 不写缓存**（缓存写在成功路径上）。⚠ **只认正整数**：小数（如 `0.5`）会让 `[ -gt ]` 报错并被 `2>/dev/null` 吞掉 ⇒ 与「未设」同路 —— 冒烟**别用 `0.001`**（那等于什么都没验）；此契约由 `tests/xyce_timeout.rs` 的 (e) 钉死。
- **单电路墙钟 `TL_MAX_WALL_S`**（仅 bestfirst）：到点 = **预算用完** ⇒ 正常收工、返回已找到的最好电路，DONE 行尾追加 `stopped=wall`。
- **连续 5 次超时 ⇒ 中止该电路**：与上一条**语义相反** —— 这是判定**仿真器/环境故障**：该电路按既有失败路径记一行 `error=连续 5 次仿真超时…`，批跑继续下一个电路，已跑完的组照旧可分析。⚠ 超时**不是静默丢弃**：照旧写 `true_delay=NA, error=Simulation timed out: …` ⇒ 分析器的 NA 计数如实报出。

### 23.5 ⚠ 检查点只在**组边界**（本弧线唯一一处主动收紧口径处）

- 为什么不能在候选循环中间 `break`：`gnn_current.csv` 的 1+n 行是 `rank_window` 在 **Pass 2b**（该组候选循环**之前**）写的 ⇒ 一旦开跑某组，1+n 行已落盘；若在组内中断，该组 `gnn_shadow.csv` **只剩前缀** ⇒ ①18.6.1 的前缀校验仍会通过（它本来就容忍未评估后缀），但 ②18.7.0 的 R1/R2 块要求 `pos` 集为**全集**，该组会被判不合格 ⇒ 分母静默变小。故**规矩是「每个开跑的组都必须完整」**，代价 = **最坏过冲一个在飞组**（≤ 12 候选 × 单次墙钟）。「连续 5 次超时中止」同样在组边界收工 ⇒ 不会继续开新组。
- ⇒ 组边界检查同时保证：墙钟到点时**末组仍完整**（这条正是服务器正面验证第 4 趟要看的）。

### 23.6 驱动：磁盘闸门 + `timeout` 前置检查（18.7.1）

- **磁盘闸门**（`run_shadow_batch.sh` 的 `0d`，`SHADOW_MINFREE_GB` 默认 50）：放在**什么都没发生**的位置（旧树未归档、`$TREE` 未建、报告 CSV 未碰、分片未起）⇒ 中止**零副作用**；`df` 取不到数就**放行**（不因为取不到而挡住正常跑）。⚠ 与 `shadow_campaign.sh:263-266` 那道是**两道闸**，各管各的调用路径 —— 上一轮曾说「驱动自带 50G 闸门」，那是把 campaign 的那道记到驱动头上了，此处更正。
- **`command -v timeout` 前置检查**：`MODE_ENV` 带 `XYCE_TIMEOUT_S` 却没有 `timeout` 时**开跑前拦住** —— 否则模板里的 `timeout` 会以 127 收场、被 Rust 当成「仿真跑挂了」，「连续 5 次超时即中止」的保险**静默失效**。
- `shadow_campaign.sh` 加 `export SHADOW_MINFREE_GB="$MINFREE_GB"`（原来只是普通赋值 ⇒ 子脚本读不到 campaign 的 `--minfree`）。

### 23.7 定档算术（2026-09-25，据 level4/ADD4_OVF 实测）

实测条件：`TL_ONLY=level4/ADD4_OVF`、`TL_MAX_STEPS=2`、新缓存目录、冷缓存。读数：`real 13m41.709s`、`evals=29`、`window tries 3`、`sum group_size 28`、缓存文件 3,512、DONE 行 `accepted=2 iters=2 evals=29 failed=0 backtracks=0 max_depth=2`。⇒

| 量 | 值 | 算法 |
|---|---|---|
| 单候选评估 | **≈28.3 s** | 821.7s ÷ 29 evals（这是**最慢电路**的上界口径） |
| 单步（轮） | ≈410 s | 821.7s ÷ 2 iters（28 候选 ÷ 2 轮 ⇒ 每轮 ~14 次评估） |
| 平均组大小 | ≈9.3 候选/窗 | 28 候选 ÷ 3 `window tries` |
| 单候选缓存文件 | ≈121 | 3512 ÷ 29 |
| 全量规模 | ≈46 × 40 × 124 ≈ **23 万次仿真** | 这也是磁盘闸门存在的理由 |
| `TL_MAX_STEPS` | **40** | 预算 ≈ 5,000 次仿真/电路 |
| `XYCE_TIMEOUT_S` | **1800** | ≈最慢**单次 Xyce 调用**（0.28 s）的 **6400 倍** ⇒ 正常电路绝不误杀（⚠ 本节原先写的「≈最慢单候选的 64 倍」**分母用错了**：1800 限的是一次调用、28.3 s 是一个**候选**（≈100 次调用）；2026-09-25 实测订正，见 §23.9） |
| `TL_MAX_WALL_S` | **21600** | 必须 > 40 × 410 = **16400** ⇒ 健康电路跑满 40 步 |

⚠ **「14.5 窗/步」这个旧说法在本趟读数里对不上（待复核）**：本趟是 3 `window tries` / 2 轮 = **1.5 窗/轮**、28/3 ≈ **9.3 候选/窗**、28/2 = **14 候选/轮** ⇒ 14.5 更像「候选/轮」而不是「窗/步」，但旧记录没写清分子分母、**不据此改任何口径**（`TL_MAX_STEPS` 的预算是**轮**，与窗数无关）。

⚠ **墙钟为什么必须大于 `40 × 410s`**：一旦真的咬合，两臂（换 ckpt）**砍的位置不同** ⇒ 该电路**失去配对**。故 21600（≈1.3 倍余量）不是「越大越好」，而是**保配对的必要条件**。全量正常路径的墙钟 ≈ 最慢电路 40 步 ≈ **4.6h**（驱动 level4 是**一电路一进程**并行 ⇒ 全量就等它）。最坏界（只在仿真器真卡死时才到）：21600s + 一个在飞组（≤12 × 1800s）= **12h**。

### 23.8 读数

✅ **服务器正面验证四趟全绿（2026-09-25）**：正常路径 40 步**零 NA 行** / 单次超时触发 / 连续超时中止 / 墙钟触发 `stopped=wall` 且**末组仍完整**（`pos` 全集不变式没被切碎）。逐趟读数与三个坑、外加一次**单位口径订正**（§23.7 那行「≈最慢单候选的 64 倍」把「一次 Xyce 调用」写成了「一个候选」）见 §23.9。全量 12 分片 2026-09-25 19:54:32 启动（`Rust 源指纹 1f7d89fac6dc`）；**R1/R2（严格 + 宽松、四行分层）＋ 换 ckpt 第二臂仍待读数** —— 命令与判读见 OPERATIONS §6.9，未决项登记在 `docs/OPEN_ISSUES.md` I23/I24。

### 23.9 正面验证四趟实测与单次仿真的真成本（2026-09-25，服务器 `~/NetlistOpt`）

验证脚本 `scripts/diag/_t_bfval2.sh`（**第二版**；第一版 `_t_bfval.sh` 的 leg (b) 因坑②作废）。每趟**独立 out root ＋ 独立 `XYCE_CACHE_DIR`**（防冷/热缓存混淆读数），基线 env `GNN_SHADOW=1 TL_BESTFIRST=1 TL_MAX_STEPS=40`。

| 趟 | 变量 | 验什么 | 实测读数（取自日志／分析器） |
|---|---|---|---|
| `n` | `TL_ONLY=level0/AND2 XYCE_TIMEOUT_S=1800 TL_MAX_WALL_S=21600` | 正常路径零变化 | `rc=0` / 157 s / `DONE mode=bestfirst accepted=21 iters=40 evals=5539 failed=0 backtracks=19 max_depth=21`；**无** `stopped=`；`NA_rows=0`；分析器 **638 窗全 0** |
| `w` | 同 `n`，但 `TL_MAX_WALL_S=120` | 墙钟咬合（组边界） | `rc=0` / 120.0 s / `iters=28 evals=3797 accepted=15 backtracks=12 max_depth=15` + **`stopped=wall`**；分析器 **436 窗全 0** ⇒ **末组仍是完整的 1+n 行** |
| `t` | `TL_ONLY=level4/ADD4_OVF XYCE_BIN=$HOME/slowsim.sh XYCE_TIMEOUT_S=1 TL_MAX_STEPS=2 TL_MAX_WALL_S=300` | 单次超时**在真管线里**触发 | `rc=0` / `wall_s=2`（`FAIL` 出现在 1.12 s）/ `FAIL …: simulation timeout: cmd = …`；NA 行 1 + error 行 1；批次**继续**（rc=0） |
| `x` | 同 `t`（跑到组尾） | 连续超时中止该电路 | `rc=0`；**修前** `连续 0 次仿真超时（XYCE_TIMEOUT_S）⇒ …（已跑 9 步、backtracks=2）`、NA 行 28 —— **行为对、数字错** ⇒ **17.6.1 修后**复跑 `连续 20 次仿真超时（XYCE_TIMEOUT_S）⇒ 判定仿真器/环境故障、中止该电路（已跑 11 步、backtracks=3）` |

`n`/`w` 的「全 0」= 分析器整节读数全 0（`配对/覆盖(pos 集非全集) 0`、`current 秩或真值缺 0`、`候选秩缺 0`、`候选真值缺/tc 不等 0`）。

**⚠ 数字读法**：`x` 印的 **20** 是**中止时**的连续次数（置位后在每次新的连续超时上覆写）⇒ 含义是「该组内连着 20 次都超时」，**不是**「第 5 次就停」；判定阈值仍是 `>=5`（首次置位於第 5 次）。**17.6.1 修的正是这个数字**：原实现在组尾才读 `consec_timeouts`，而置位→组尾之间只要有一次**成功**评估（回退命中旧缓存也算成功）就把它清零 ⇒ 印成「连续 0 次」。修法：置位时**同时**记下当时的计数（`timeout_abort_at: Option<usize>`），计数与判定分开存。

**单次仿真的真成本（这行订正 §23.7 的单位口径）**：`XYCE_TIMEOUT_S` 限制的是**一次 Xyce 调用**；§23.7 的 28.3 s 是**一个候选**（= Σ_口(可翻转该口的输入脚数×2) 次调用）。实测：`level4/ADD4_OVF`（214 只 BSIM-CMG、757 未知量）单次 **≈0.28 s**；`level0/AND2` 单次 **≈0.045 s**（10 步 1240 次 ≈ 56 s）；10 秒直接探针复核 **0.303 s**。⇒ ① `1800 ÷ 0.28 ≈` **6400 倍**（**不是** §23.7 写的 64 倍）⇒ **该机制在健康机器上实际不可达**，它挡的是「仿真器真卡死」那类形态，不是「慢电路」；② 全量墙钟的真实压力**全在 `TL_MAX_WALL_S`**，与 `XYCE_TIMEOUT_S` 无关（§23.7 的定档算术不受影响）。

**三个坑（都写进 OPERATIONS §6.9 第 2 步）**：
1. **整数守卫**：模板用 `[ "${XYCE_TIMEOUT_S:-0}" -gt 0 ] 2>/dev/null`，**只认正整数** ⇒ `0.001` 等于「没设」，**1 秒是最小可设值**。
2. **1 秒 > 一次真仿真**（0.045 s）⇒ `XYCE_TIMEOUT_S=1` **根本测不出超时**（第一版脚本据此白跑一趟；那趟反而成了有效的**反面对照**：大批真实评估、`timeout_hits=0`、零误杀）。要正面触发必须**把仿真拖慢**：用 `XYCE_BIN` 包装器（模板里 `XYCE_BIN="${XYCE_BIN:-<真 Xyce 路径>}"`，全 crate 只此一处引用 ⇒ **零代码改动**的合法手段）。
3. **分析器必须跑在 venv 里**：裸 `python3` 无 numpy ⇒ `ModuleNotFoundError`、`rc=1`、**零读数**（看起来像「树是空的」，极易误判）。正确调用：工作目录 `~/-project`，`$HOME/venv/bin/python3 scripts/diag/_shadow_analyze.py --root …`。

**两个环境闭环证据**：
1. **10 秒直接探针**（把渲染后的 `cmd_expr_…_fall.sh` 直接跑）：默认 `rc=0`（0.303 s）/ `XYCE_TIMEOUT_S=1` + 慢包装器 ⇒ `rc=42`（1.002 s，stderr 带 `XYCE_TIMEOUT: killed after 1s`）/ 慢包装器但**不设**超时 ⇒ `rc=0`（5.003 s）。⇒ env 确实传到了渲染脚本、退出码 42 契约成立、**默认分支逐字不变**。
2. **`XYCE_CACHE_DIR` 覆盖真生效**：新目录落到 **11,858** 个文件，而默认缓存目录 `temp_sim_cache/` 的 mtime **未被触碰**（仍是 09-23 00:08）⇒ 全量写的是新目录、旧缓存安全。

**跨机对账**：传输包 `~/nlo1760.tar.gz` sha1 `b54c5ea4dce98cb02a64da14f3e7142f7db2f8fb`、清单 `nlo1760.sha1` sha1 `1f7d89fac6dcbc5a41f5d00632d6d595ed07c2fe`、`sha1sum -c` ⇒ `OK_lines=56`、`RUST_FP=1f7d89fac6dc`（== 清单 sha1 前 12 位）、`build_rc=0`。全量启动行 `Rust 源指纹 1f7d89fac6dc` 与之一致 ⇒ **跑的就是验收过的那棵树**。
