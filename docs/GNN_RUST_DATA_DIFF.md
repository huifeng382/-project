...
4. **`Best midpoint` 无判别力，且判据与部署反序（⚠ 17.2.6 更正判据名）**：`Best midpoint` 由加权 `score`（`train_sweep.py:978`，`100·r3+50·r2+0.3·sp−0.2·regret+0.1·cap`，recall@3 主导）选出，**不是** `BEST_METRIC=smoothed_rel_err`（后者是 **`best_model.pt`** 的判据，两者是不同 selector）。同一 selector 在 42m4 挑中五点最优、在 42b 挑中五点最差（1/2）；进一步查 42b 训练侧逐 epoch（§13.7.3）得**该 `score` 序与 Rust 部署序 Spearman = −1（五点完全反序）**。**不得默认服务 Best midpoint。** 附第二次命中「训练侧 Spearman 高 = 部署差」（42m4 ep200 训练侧 Sp 次高 → 部署 13.78% 该臂最差）。
...
1. **目标是 delay，损失里没有排序项**：`train_sweep.py:84-91` —— `log10(data.y)` 逐行、Huber(delta=0.3)、按开关脚 `PIN_WEIGHTS` 加权取均值；`RANK_LOSS_W=0`、KD 关闭（纯拓扑臂）。**组内排序只是被测量的量，不是被优化的量**（回归副产品）→ 它在排序轴上早饱和是**必然而非意外**。（另一条 idsavg GNN 线目标是 **ids**、指标 val_R²，与本节的 delay GNN 无关。）
...
| ② | `struct_prior` | `[transistor_count, #{g∈ct: 'SC_AND'∈g ∧ 'SC_AND_'∉g}, #{g∈ct: 'SC_INV_WIRE'∈g}]`，`ct=cell_types_json`（`data_loader.py:710-719`，`USE_STRUCT_PRIOR=True` @ `config.py:107`） | **整个传 `None`**（`serve.py:286`）→ `model.py:110` 的 `if struct_prior is not None` 为假 → 结构残差整块静默跳过 |
...
|---|---|---|---|---|---|---|---|
| **① 全部批（部署真值）** | **714** | **53.1%** | **74.4%** | **69.6%** | **97.6%** | **7.23%**（中位 3.88%） | **1.66%** |
| ② 按候选池去重（同电路同 sig） | 161 | 50.3% | 68.9% | 75.2% | 95.0% | **4.76%** | 0.94% |
| ③ 按电路宏平均（每电路等权） | 18 | 60.3% | 79.5% | 76.9% | 96.9% | 5.67% | 0.75% |
| ④ 排除最大电路 `level2/DEPTH_MIX` | 176 | 48.3% | 65.3% | 73.3% | 90.3% | 5.11% | 0.76% |

其余 ① 读数：cap2 95.55%、Spearman 0.546、hi_spread 591 集；诊断位 31/20/271/0/0（hi 子集 14/4/0/0），其中**量纲混合守卫 0/714**——守卫比对的是「一个集内秩 vs 原始延迟」的混装，而**集内不可能混**（一个窗口要么全 pre_rank 命中、要么全 evaluate 兜底），故它 0 是正确读数，**不构成对池化失真的检测**。

③ 只有 **18 个单位**（不是 46）：46 个 CSV 里 28 个一个合格集都没有。④ 与 ① 可反解最大电路自身质量：严格 k3 = (74.4×714 − 65.3×176)/538 = **77.4%**（高于全局 74.4%），选择遗憾 = (7.23×714 − 5.11×176)/538 = **7.92%**（最差）→ **DEPTH_MIX 的问题在「权重」不在「质量」**（538/714 = 75% 权重压在 1/18 个电路上）。
...
### 15.6 判据的稳健性：严格 recall 稳，「选择遗憾 ≤5%」不稳

**严格 recall@3 ≥90% 四种口径全不达标**（74.4 / 68.9 / 79.5 / 65.3；宽松 97.6 / 95.0 / 96.9 / 90.3）→ **「GNN 只做启发式粗排」与口径之争无关，成立**。
**选择遗憾 ≤5% 只在 ② 过**（4.76%），① 7.23% / ③ 5.67% / ④ 5.11% 全在线上 ±0.7pp 内 → 诚实读法是**「≈5%，临界」，不是达标**。

两个已知的读表陷阱：**（a）③ 宏平均混了 n 不同两类集** —— n=4 的集其严格@3 随机基线是 75%，n=8 的是 30%，宏平均把两种难度等权；需 n≥8 分层版（**代码未改，待批**）。**（b）宽松 k3 在 n=4 集上定义性饱和** —— 真前3 与预测前3 各占 4 中的 3 → 必然相交 → 恒 100%；对全体 宽松@3 的影响小（f=20% → 97.0%、f=30% → 96.6%，实测 97.6% 在带内），但**读这个数时必须记住**。
