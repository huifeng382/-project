...
4. **交付含义**：Rust 粗筛模型应走**纯拓扑 no-wave（nowave）路线**；USE_IDS_AVG_APPROX 特征线对 serve 粗筛整体负贡献（训练侧也白搭，见 PROJECT_LOG 三兄弟训练对比）。牵连：v2kdwave42iaa42 蒸馏学生用 iaa 特征，其 Rust 前景待训完单验。
