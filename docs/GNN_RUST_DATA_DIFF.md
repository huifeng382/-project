...
**(a) 两臂的特征表也不同 —— 数据与特征同一次改动，本趟分不开归因。**
基线 in_dim **14**（I9 记载，17.3.11 run_stamp 实读）；V3 in_dim **45**；
而 18.5.0 的启动日志写 `1 static + 31 arcs` ⇒ **14（静态）+ 31（弧特征）= 45** 自洽。
所以只能说成 **「V3 交付栈（新数据 + 31 弧特征）相对 V2 交付栈在部署电路上严格@3 退步」**，
**不能**写成「V3 数据本身差」。要分开需要一个「V3 数据 × 无弧」的臂，现在没有；
**`v3wave42` 也不分不开**（弧特征在 `USE_TRANSISTOR_WAVE=False` 下照载，同一条日志可证）。
