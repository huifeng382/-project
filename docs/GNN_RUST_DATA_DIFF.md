...
### 12.4 给生成方 / 下一次数据的要求清单（16.11.17 更新：基于 V1-V7 差距归因）

> **60w 新数据的生成要求**。当前数据缺陷 → 新数据改进点如下（按优先级）。

#### 缺陷 1：深度 OOD（最严重，recall 下降主因）
- **实测**：训练数据（full/rest/m4）深度**全部 ≤5（>6 占比 0%）**；Rust 深电路（ADD4_OVF/OVF 表达式深度 8-12）recall@3 仅 12-20%；recall@3 随规模递减（小 64% > 中 58% > 大 52%）。
...
#### 缺陷 2：规模覆盖不足
- **实测**：Rust tl_opt 候选门数中位 36（范围 1~73）；训练数据门数中位 4-15（max 23）。
- **要求**：门数覆盖 25~80+，晶体管 100~400+；**与深度解耦**——既要有「宽而浅」（多数门并联）也要有「窄而深」（少门串联）。
...
#### 缺陷 4：候选集内「近延迟对」与「可分辨 spread」
- **实测**：Rust 候选集 spread 中位 47.7%（9.1% 的集 <5%）；训练按 expr 组 spread 中位 157.8%（>10% 为主）。训练数据**缺少 spread<10% 的近延迟候选对**，模型没学过细粒度分辨（recall@1 仅 11%）。
- **要求**：① 含**组内 spread <10% 的近延迟样本**（同功能小变体，延迟差 <10%）；② 也要含 spread 20-60% 的可分辨样本（Rust 实际有 48% 中位）。
- **关键**：训练评估与 Rust 一致的**组内排序任务**（不是全数据集延迟回归）。

#### 缺陷 5：I/O 形状（已覆盖，维持）
- Rust 46 电路 18 种形状已 100% 覆盖（full+rest+io+m4）——**非瓶颈**；m4 补的多输出形状 Rust 用不多。
- **要求**：维持全形状，**优先单输出**（Rust 主战场 (1,1)(2,1)(3,1)(16,1) 等）。

#### 缺陷 6：样本量与分布
- 现 full+rest 69,432 行但门数小电路密集；m4 只 108,720 行 5 形状。
- **要求（60w）**：重点投在**深电路 + 大电路**区间（缺陷 1/2），而非继续铺小电路；各 expr 组 ≥10 变体（保持 MIN_GROUP_SIZE 过滤后可用）。
