...
5. **判据与部署口径反序**：`score` 序 vs Rust 序 **Spearman = −1**（五点完全反序）；同名端点也反转（训练 r3 最高 ep100 → Rust 严格 k3 最低 32.1%；训练 r3 最低 ep150 → Rust 严格 k3 最高 34.9%）。机制 = `score` 被权重最大、却最不转移的 recall 主导。**⚠ n=5，五点全反序可能纯属巧合，不作定律**；另有 `42b` ep100 / `42m4` ep200 两处独立同向命中。
