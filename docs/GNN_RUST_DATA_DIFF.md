...
**⚠ 附带影响（17.4.1）：全库 `train_sweep.py:NNN` 引用整体 +55。** 17.4.1 在 `src/train_sweep.py:25` 处插入了 `midpoint_epoch_of`/`pick_midpoint` 共 **55 行** ⇒ 该文件**第 24 行以下所有钉死行号平移 +55**。已知受影响引用：本报告 `:952`（`train_sweep.py:978` → 现约 `:1033`）与 `:960`（`:84-91` → 现 `:139-146`）、`PROJECT_LOG.md:921`（`:84-91`）、`:956`（`:815-825` / `:978-984`）、`GNN_CODING_LESSONS.md:19`（`:142-149, 192` → 现 `:197-204, 247`）。**已改**：`OPERATIONS.md:212` 与 `PROJECT_LOG.md:1100/1103`（政策操作性引用，改成**按锚点找** `grep -n 'cap2={score' src/train_sweep.py` —— 行号必然再漂，锚点不会）。**未改（待用户定夺，见下）**：其余为历史分析记录，改行号需连同结论一起复核。

**🔴 另有比行号更重的发现（待定夺，未擅改）**：`train_sweep.py:1113` 的注释写着「**17.3.7 换判据：原 score（100·r3+50·r2+0.3·sp−0.2·regret+0.1·cap，recall@3 主导）**」—— 即 **Best midpoint 的加权 `score` 判据在 17.3.7 就已被 capture2 取代**（现 `score = mrk.get('capture2_pct', ...)`）。因此本报告 `:952` 与 `PROJECT_LOG.md:956`（那条「⚠ 纠一处判据名错误 / 加权 score 选出」的更正）**描述的是已不存在的代码**：它们说「`Best midpoint` 由加权 score 选出、`smoothed_rel_err` 才是 `best_model.pt` 的判据」—— 前半句自 17.3.7 起不成立，且 `BEST_METRIC` 这个名字也已改名 `BEST_MODEL_METRIC`。**核心结论（该 selector 序与部署序反序）未必失效，但它挂在哪个判据上需要重述**，且与 §15.6 的 capture2 判决（❌ 不采纳）是同一件事的两面 ⇒ **留给用户与 `BEST_MODEL_METRIC` 定档一并处理**，本次只记不动。
