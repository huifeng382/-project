...
| `XYCE_TIMEOUT_S` | **1800** | ≈最慢单候选的 64 倍（正常电路绝不误杀） |
...
🟡 **待服务器正面验证**（四趟：正常 40 步零 NA 行 / 单次超时触发 / 连续 5 次超时中止 / 墙钟触发 `stopped=wall` 且**末组仍完整**）＋ 全量 12 分片 → R1/R2（严格 + 宽松、四行分层）＋ 换 ckpt 第二臂（近乎全命中）。命令与判读顺序见 OPERATIONS §6.9；组边界检查点的最坏界与配对风险同时登记在 `docs/OPEN_ISSUES.md` I23/I24。
