...
3. **B=0.7912（R4 共享+腿 n<8 + R6 专化+腿 ge8，按 n_t 路由）即 R² 线部署形态**。剩余动作只剩 **#35 多 seed 集成**（波1 R4 SEED1-3 跑中，测 B 的 +0.0029 散布）出稳健部署数 → 之后转 **serve 指标 / 相对误差**判定（不再投 R² 训练）。
...
- ✅ **R² 优化线已收口（2026-09-08，§4.8 定案 A）**：桶分解 + 锥轴探针（零训练，`scripts/diag/_t_{bucket_cost,depth_probe,cone_probe}.py`）→ headline ~0.79 相当成分 = idle/active 群体分离；**激活门 per-class R² ~0.5–0.6（深堆叠 & 浅层激活同墙）→ exact-log1p pooled R² ≥0.85 不达**。B=0.7912 即 R² 线部署形态；不再投 R² 训练，之后转 serve 指标/相对误差判定（#35 出集成稳健数后收尾）。详见 directions §0.9。
