...
> 状态：服务器全量基线（17.0.1）、容量+锥体 A2/B2（17.0.6/17.0.7）、**长程对照 B2long/B2v2long（17.0.10）均完成判定**，见 §4.4/§4.5；**17.0.14 收口 regime 复现 + 候选 ①/②' 裁决（R1-R4）已判定**，见 §4.6；**n≥8 专化 R5/R6 + 复合路由 B=0.7912（全口径新最佳，17.0.17/19/20）已判定**，见 §4.7；**R² 优化线已收口（2026-09-08 桶分解 + 锥轴探针，§4.8）= B=0.7912 即 R² 线部署形态，exact-log1p pooled R² ≥0.85 不达，转 serve 指标判定**；**多 seed 集成 #35 已定档（2026-09-10，§4.9）：B=0.7912 居 4×4 复合分布正中央、4×4 集成复合 0.7935，集成增益仅 +0.0023**。
...
- **定性**：锥体/腿/专化每层机制 headline 各 +0.002~0.005，叠加至 0.7912；再往上撞的是激活态结构墙。**多 seed 集成（#35）已定档（§4.9）**：B=0.7912 居 4×4 复合分布正中央（mean 0.7914 ± 0.0020）→ +0.0029 非 seed 噪声；4×4 集成复合 0.7935（集成增益 +0.0023）。

### 4.8 R² 优化线收口：桶分解 + 锥轴探针（2026-09-08 · 零训练纯诊断 · 定案 A）

**背景**：B=0.7912（§4.7）后评估"再往 0.85 投"是否值。零训练，纯读 R4/B resid 三探针（`scripts/diag/_t_{bucket_cost,depth_probe,cone_probe}.py`，服务器 560,242 test 行）。

**测出的事实**
- **桶分解**（`_t_bucket_cost`，R4 单 & B 复合两张同表）：全局 SSE 份额 **n≤2 ≈ 82%**（378k 行 = 67.5%，mean_y 0.374）→ 若该桶归零 headline 上 0.96；n3-7 0.13%（5.3% 行）；**n≥8 ≈ 4%**（152k 行 = 27.2%）→ 深桶即使完美也只到 0.8258。**表面矛头全指 n≤2**。
- **锥轴探针修正**（`_t_cone_probe`；正确轴 = 距本次切换 src 的锥内位置 `cone_down`，原 `_t_depth_probe` 用静态逻辑深度 = 错轴，见下）：n≤2 的 0.8022 是**两群混合**——
  - **不在切换锥（out-cone idle/漏电群）** 231,948 行 61%，mean_y 0.101（近零目标）：R² 0.0879 纯口径假象，SSE 占 n≤2 仅 14.4%；
  - **在锥内（in-cone 切换门）** 146,340 行 39%，mean_y 0.807：池内 0.6455 本身又被 mean 随 cone_down 的趋势抬高，**分箱后 within-bin 只有 0.26–0.50**。
  - **全局误差最大单块 = cd 0.4–0.6 中锥高电流门**（68,349 行，mean_y 1.110）：占锥内 SSE **69.4%**，within-bin R² 仅 0.50。
  - corr(|残差|², cone_down) = **+0.213**、与静态 depth +0.211 → 传播/埋没轴对残差解释弱。
- 桶分解与锥探针结合读：n≤2 桶的 ~82% SSE 里 ~86% 在 39% 的 in-cone 激活门；而 in-cone 的分箱墙 ~0.5 与深堆叠墙 ~0.6 同源（激活态，§0.5）。

**裁决（定案 A = R² 线收口）**
1. headline ~0.79 的相当成分是**群体分离**（idle-vs-active 判别 + 电流量级分层），不是对激活门电流的精细预测；**激活门 per-class R² ~0.5–0.6，跨 深堆叠（§4.7 0.60）& 浅层激活门（分箱 0.26–0.50）都是墙** → sim-free 现有特征族（锥体/腿/专化/静态结构）已达信息墙。
2. **exact-log1p pooled R² ≥0.85 不达**；唯一机制自洽但探针支持弱的未试特征（logical-effort 沿锥逐门传到的到达 slew/load，serve 可行）只够期待 0.82–0.83 且须砍掉 cd0.4–0.6 主残差块 20–30%，**不入主动池，收档于此**（若 serve 判定后仍要博，再议）。
3. **B=0.7912（R4 共享+腿 n<8 + R6 专化+腿 ge8，按 n_t 路由）即 R² 线部署形态**。剩余动作只剩 **#35 多 seed 集成**（已完成，§4.9：B 居 4×4 分布正中央、集成复合 0.7935）→ 之后转 **serve 指标 / 相对误差**判定（不再投 R² 训练）。

**方法论教训（入档）**：原 `_t_depth_probe` 用静态逻辑深度（主输入最长路径）当"埋没度" = 错轴。本任务**一行 = 单次开关事件**，正确传播轴 = 距本次切换 src 的锥内位置（cone_down）；深堆叠门的低 R² 是激活态墙，不是"静态埋深"墙。结论以 cone 轴为准，depth 轴读数作废。

### 4.9 多 seed 集成定档（#35 · 2026-09-10 · 4×4 全配对 · 零训练）

**背景**：§4.7 的 B=0.7912 是单 seed（seed0 共享 + seed0 专化）口径，需测它是否落在 seed 噪声内 + 出集成稳健数。17.0.21 加 SEED 旋钮（默认 0 = 历史逐位不变；GSZ 无 shuffle + 切分 RandomState(7) → 各 seed resid 按行逐位对齐可直接平均，seed 只改 torch 初始化 + dropout）。波1 = R4 共享+腿 SEED1-3（`resid_R4e*_pin_17021.parquet`）；波2 = R6 专化+腿 SEED1-3（`resid_R6e*_pin_17021.parquet`）。分析 = `scripts/diag/_t_ensemble.py`（560,242 test 行）。

**单模型散布**
- 共享+腿 full R²：0.7883 / 0.7867 / 0.7926 / 0.7888 → mean **0.7891 ± 0.0022**
- 专化+腿 ge8 R²：0.6102 / 0.6011 / 0.6091 / 0.5980 → mean **0.6046 ± 0.0052**（seed0 最高；专化侧抖动 ≈2× 共享侧）

**4×4 全配对复合**
- mean **0.7914 ± 0.0020**（range 0.7890–0.7951；seed0+seed0 = 0.7912 与 §4.7 存档一致）
- 对角（同 seed 共享配同 seed 专化）mean 0.7914 = 全配对 mean → **无配对效应**，成绩全由机制扛，谁跟谁搭都一样
- **B=0.7912 落在分布正中央**（≈mean，略低）→ 非 seed 手气，+0.0029 机制收益可复现
...
| 口径 | full test R² |
|---|---|
| 单 seed 复合 B（seed0+seed0） | 0.7912 |
| 4×4 复合期望 | 0.7914 ± 0.0020 |
| **4×4 集成复合（可部署多 seed 数）** | **0.7935** |
| 机制增益（ge8 专化 vs 共享，池内） | +0.0324 |

**裁决**：集成增益仅 +0.0023 → 多 seed 对 §4.8 的 0.85 墙无任何松动，侧证 A 裁决。**R² 线最终形态 = B 双模型按 n_t 路由，单 seed 0.7912 / 4×4 集成 0.7935**；专化侧 seed0 是四者最高（0.6102 vs mean 0.6046），但均摊后对 headline 影响 ~0.0003 级，不改变定档。

**Caveat（入档）**：GPU 侧 scatter_add 聚合非确定性 → 同 SEED 重跑 R² 有 ~0.005 抖动（首轮 env 名写错那批 0.6044/0.6041/0.6080 vs 本轮 0.6011/0.6091/0.5980，env 名不影响训练数值）。故多 seed 散布含"跑批噪声"，非纯 seed 方差——解读为"部署时你能拿到的总方差"即可。另：首轮波2 因 env 名误写（`CKPT`/`DUMP` 而非代码实读的 `SAVE_CKPT`/`DUMP_RESID`）只出 R² 无 resid，17.1.1 修 `_t_ensemble.py` 的 n<8 切片 bug 后重跑落盘。
...
- ✅ **多 seed 集成（17.0.21 #35，已定档 2026-09-10，§4.9）**：SEED 旋钮（默认 0 = 历史逐位不变；GSZ 无 shuffle + 切分 RandomState(7) → 各 seed resid 按行逐位对齐可直接平均）。4×4 全配对复合 mean 0.7914 ± 0.0020（**B=0.7912 居正中央 → +0.0029 非 seed 手气**）；对角 = 全配对 mean（无配对效应）；集成复合 0.7935（+0.0023）。专化侧 seed0 0.6102 为四者最高，均摊后对待部署 headline 影响 ~0.0003 级。
- ✅ **R² 优化线已收口（2026-09-08，§4.8 定案 A）**：桶分解 + 锥轴探针（零训练，`scripts/diag/_t_{bucket_cost,depth_probe,cone_probe}.py`）→ headline ~0.79 相当成分 = idle/active 群体分离；**激活门 per-class R² ~0.5–0.6（深堆叠 & 浅层激活同墙）→ exact-log1p pooled R² ≥0.85 不达**。B=0.7912 即 R² 线部署形态；不再投 R² 训练，之后转 serve 指标/相对误差判定（#35 集成稳健数已出：§4.9 集成复合 0.7935，收尾）。详见 directions §0.9。
