...
| `batch_v2_rest` | 35,183 | 568,090 | 3,013 | expr9500-12512 | 满量补齐（分桶合成 + v2_exprs 未覆盖部分） |
| `batch_v2_m4` | 1,800 | 108,720 | 150 | expr13000-13149 | **V3.2 补充：5 个缺失 benchmark 形状**（见下） |
| **合计** | **49,910** | **804,006** | **4,148** | — | 单 corner `s02p0_l01p0` |
...
### 1.1 batch_v2_m4（V3.2，2026-08-29）

`DATA_SPEC_V2_ISSUES.md` 第五节要求的 5 个缺失 Rust benchmark 形状，各 **30 组 × 12 变体**：

| 形状 | 组数 | 电路 | 行/电路 | 行数 |
|---|---|---|---|---|
| 9入6出（ADD4_OVF 形状） | 30 | 360 | 108 | 38,880 |
| 8入4出（ENC8 形状） | 30 | 360 | 64 | 23,040 |
| 7入4出（ALU2 形状） | 30 | 360 | 56 | 20,160 |
| 5入5出（SHIFTER4 形状） | 30 | 360 | 50 | 18,000 |
| 4入3出（ENC4 形状） | 30 | 360 | 24 | 8,640 |
| **合计** | **150** | **1,800** | | **108,720** |

- **组内功能等价**：变体由函数保持的结构改写生成（交换/结合/幂等展开/双重否定），逐输出真值表穷举一致；
- **结构合法**：修复了 Inv 套 And/Or 的非法形式（曾产生 `SC_INV_AND` 使输出恒值 → 该输出全部 tb FAILED），
  全部变体 Inv 仅套 Var/Inv；
- **行数完整**：每电路恰 `2×N×M` 行（0 缺行）；组大小全部 12（0 组 <10）；
- **expr 编号 expr13000+** 与 rest（expr9500-12512）不重叠。
...
| 输入分桶 3~4 | ~25% | 12,630（25.3%） | ✅ |
| 输入分桶 5~8 | ~25% | 12,978（26.0%） | ✅ |
| 输入分桶 9~16 | ~25% | 12,506（25.1%） | ✅ |
| 多输出占比 | ≥20% | 12,795（25.6%，输出 2: 8,251 / 3: 3,104 / 4: 720 / 5: 360 / 6: 360） | ✅ |
| **G1 缺失形状** | 5 个 benchmark 形状全覆盖 | 9入6出 / 8入4出 / 7入4出 / 5入5出 / 4入3出 全部 OK（`_check_v3_data.py` 形状视图） | ✅ |
| corner | 单 `s02p0_l01p0` | 全部一致 | ✅ |
| 每电路行数 | 2 × N_in × M | 0 不匹配（P6 剔除后） | ✅ |
| sc_expansion | 全部 SC_ 名可展开 | 19,372 名，缺 0、展开为空 0（映射表 24,625 项） | ✅ |

## 三、验收结果

接收方脚本 `scripts/diag/_check_v2_data.py`（四批合并，batch_v2_full + io + rest + m4）：

```
PASS 46 | FAIL 0 | WARN 1
WARN: ids_charge ≠ ids_avg — 复制残留 29/261,446 激活管（0.011%）
```

**WARN 说明**：`ids_charge` 已按 P5 修复公式 `ids_peak × ids_rise_time / 1000`（fC）重算，
并统一为 **4 位小数**（2026-08-28 三批重收集，此前 2 位小数取整巧合占比 1.15% → FAIL）。
残留的 ~0.01% 为窄脉冲晶体管（仅在翻转窗口导通）数值上 `peak×rise/1000 ≈ avg` 的物理巧合，
...
coverage_report 全部 100%。`scripts/diag/_check_v3_data.py` 形状视图：18 种 benchmark 形状全部 OK。
