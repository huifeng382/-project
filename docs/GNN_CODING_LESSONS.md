...
- **原因**：图缓存键 = `hash(graph_builder.py) + md5(各parquet 的 int(mtime)) + STRUCT_MODE`（`src/train_sweep.py:215-222` 的 `_data_mtime_hash` 与 `:264-265` 的键合成；⚠ **行号随版本漂移，别做常数换算** —— 此为 **17.4.3 时**读数，17.4.1 时为 `:197-204` 与 `:246-247`，更早记 `:142-149, 192`。偏移按位置分档（17.4.1：+55/+60/+63/+73/+87；17.4.3：+0/+18/+22/+26），见 DIFF §15.8 偏移表）。新 clone 的数据 mtime ≠ 旧数据 mtime → 键变 → 清空重建。
...
### 8.2 杀进程前必查 cwd/进程树——kill 误杀他人训练（16.11.10 血泪）
- **现象**：为释放内存 `kill -9 3280966 3280992 3280993`（想杀 v2iag42m4），结果 **v2nowave42m4 主进程（3209390）连带死亡**（DataLoader worker exited unexpectedly），13.5h 训练进度（184 epochs）丢失。
