...
| I9 | 🔶 低优先 | serve 8000 端口现挂 **v2nowave42m4 midpoint_ep250**（无近似，in_dim=14；**ckpt sha1 = 09be9a3c53b90760**，17.3.11 `run_stamp` 实读）；占 ~1.1GB 内存，下次换模型时一并停旧起新。其**部署口径（714 集 batch）**读数 = 严格@3 **74.4%** / 宽松@3 97.6% / 选择遗憾 7.23% / 两阶段 1.66%（**与历史 106 集口径的 39.6/65.1/10.87 不是同一把尺**） | OPERATIONS §5；DIFF §15.2/§15.5 |
