...
- 同口径基准（m4三兄弟 Rust，DIFF §13.2/13.3）：纯拓扑 **nowave 遗憾 10.87%**（serve 交付基线）；iag GBDT15 严格@3 44.3% / 遗憾 12.19%；iaa ⚠ 15.21%。
  - ✅ **已定案（2026-09-15 同日对照跑）**：`42b`（in=45, ep100）复跑 **14.26%**，同口径重跑交付基线 `42m4` ep250 得 **10.66%** → 历史 10.87% 被复现到 **0.21pp** 以内，**口径 / `XYCE_CACHE` / 解析脚本都可复现，10.87% 可信，42b 那 3.60pp 差是真的**。部署口径三臂单调可分：`42m4` **10.66** < `42b` **14.26** < `gnn42b` **24.00**（+GNN ids 模式 3）→ **交付基线 42m4 未被撼动**。（42b 仍赢下 A/B 全部 7 个判据，故 A/B 方向不可能翻转。）
  - ⚠ **新教训（原因待分离）**：42b 与 42m4 训练侧几乎同级（3.94% vs 3.80%）却部署差 3.60pp，头号嫌疑 = **ckpt 选点的依据是 val loss**（42b 服务 ep100 / 42m4 服务 ep250）—— 与「val loss ≠ 排序质量」那条老教训同源，这是它第二次独立命中。**⚠ 因而不要默认「服务 Best midpoint」就是部署最优**；换 ckpt 前后各跑一趟 shadow 验一下很便宜（缓存全命中，单趟 ~2.5min）。详见 PROJECT_LOG §17.1.x。
