...
- **恢复 serve**：非交付路线的模型验完，按 Step 2 换回 v2nowave42m4（`--ckpt ~/project-107-v2nowave42m4/outputs/midpoint_ep250.pt`；ckpt sha1 应为 **`09be9a3c53b90760`**、`in_features` 应为 **45**（2026-09-22 实测；**不是旧记的 14**）、**不带 `USE_IDS_AVG_APPROX`**）。⚠ **「不带 env」这句已作废（17.4.0）** —— §6.3 那三行（`GOMP_SPINCOUNT=0 OMP_WAIT_POLICY=PASSIVE KMP_BLOCKTIME=0`）**加上 `PYTHONHASHSEED=0`** 现在是**每一条** serve 启动命令的必带项，交付基线的 serve 也一样。
  - 🆕 **分析器新增「以 current 为锚的 GNN 判据质量」节（18.6.0 落地 / 18.6.1 改核心口径 = 首推真负率）**：读 `★ 核心 首推真负率`（严格 / 宽松）、**分母缺口三桶**（首推 current ＋ 首推无真值 ＋ 进分母 = 合格集）、镜像漏推率、并列数（含 `t1_mix`）。旧的逐候选 误报率 / 漏报率 已降级为**参考口径**（分母是候选不是窗）—— **别拿它当核心数**。⚠ **18.6.0 首跑那组（含 99.71%）全部作废**（校验口径把「GNN 可能说对」的窗系统性筛掉了，机理见 DIFF §22.4）⇒ 引用本节数字前先确认分析器 ≥ 18.6.1 且诊断行 `推断位置非前缀: 0`。命令见 §6.8。
- ⚠ **当前状态（2026-09-23）**：本趟（DIFF §22 的 `gnn_current` 观测）用的是 §5「其他现场」那条记录的现挂交付基线 **`v2nowave42m4 midpoint_ep250`**（该行含 PID 1434028 与 `cmdline` argv 实测证据；**判身份一律照 §6.3 实测，别拿戳判身份** —— 那是 I20 的坑、别用 `ps -o args`）。**V3 第二臂 `midpoint_ep200` 尚未跑** ⇒ 跑完按本行换回 `ep250`，并确认换上的不是 `ep300`（09-17 ckpt-sweep 末位遗留过，见 I9）。
- ~~**当前状态（2026-09-22）**~~（已过时，留档）：8000 上挂的是 **V3 `v3nowave42` ep200**（本轮 V3 shadow 换上去的，**待收**）。**恢复前先按 Step 2 换回 `midpoint_ep250`**，并顺手确认换上的不是 `ep300`（09-17 ckpt-sweep 末位遗留过 `ep300`，见 I9）；换完按 §6.3 用 `tr '\0' ' ' < /proc/$PID/cmdline` 复核 argv 里的 `--ckpt`（`ps -o args` 会被列宽截断）。
...
- **`gnn_current.csv`（18.6.0 起，`rank_window` 观测落盘；DIFF §22）**：每个候选集一行组 —— `CURRENT` 行 + 各候选行，`rank` = 「`[current]` + 本窗口全部候选」放进**同一批** `/rank` 的秩（1-based、并列取平均秩）。⚠ 与 `gnn_shadow.csv` 的 `gnn_pred`（**纯候选批**）**不可互读、不可跨文件比较**；候选真值只能按 `eval_idx` **join** `gnn_shadow.csv`（本文件候选行 `true_avg=NA` 是刻意的）。⚠ 「文件在 = 观测在」：候选全空的窗**本就不写**（`rank_window` 的空候选守卫），`ls` 少几个文件属正常。
