...
**其他现场**：
- serve：**8000 现挂 V3 `v3nowave42 midpoint_ep200`（2026-09-22 本轮 V3 shadow 换上去，待收；换前是 `v2nowave42m4 midpoint_ep300` = 09-17 ckpt-sweep 末位遗留，非交付基线，见 I9）**。**交付基线（用户 2026-09-22 决定维持）= `v2nowave42m4/outputs/midpoint_ep250.pt`**（in_dim 14；**不带 `USE_IDS_AVG_APPROX`**，但 §6.3 那三行 + `PYTHONHASHSEED=0` 必带；命令见 §6 Step 2/Step 6）；09-04 起「serve 不常驻」政策不变——确认无后续 shadow 需求可 `pkill -f 'serve_htt[p].py'` 停。
...
- 同口径基准（m4三兄弟 Rust，DIFF §13.2/13.3）：纯拓扑 **nowave 遗憾 10.87%**（serve 交付基线）；iag GBDT15 严格@3 44.3% / 遗憾 12.19%；iaa ⚠ 15.21%。→ ⚠ **这三行都是 17.2.4 前的数**（边序未知，±0.31pp 带内），见下方权威表。
  ...
  - 📌 **V3 交付栈部署读数（2026-09-22，18.5.1）= `v3nowave42 midpoint_ep200`（sha1 `51bd323382ecb9b7`，in=45 不带 env）在 714 集 batch 口径下：严格@3 75.8% / 宽松@3 97.5% / 选择遗憾 5.37%（中位 2.57%）/ 两阶段 0.58% / Sp 0.637 / 捕获率 97.33%**（hi-spread 591 集：严格@3 77.2% / 宽松@3 98.0% / 遗憾 6.24%）。对照交付基线 ep250（90.8 / 97.9 / 5.93 / 0.63）⇒ **严格@3 −15.0pp（hi-spread 集 −17.2pp）、宽松@3 持平、选择遗憾与两阶段小幅变好（四口径遗憾全部变好）** ⇒ **用户 2026-09-22 决定：交付基线维持 ep250，不切换**。⚠ 两臂 **in_dim 14 → 45**（+31 弧特征）与数据**同一次改动一起上** ⇒ **归因分不开**（`v3wave42` 也分不开：弧特征在 wave=False 下照载）。⚠ 106 集 window 口径下 V3 有 **12/106 量纲混合 ❌** ⇒ 该口径数不得引用。详见 DIFF §21。
