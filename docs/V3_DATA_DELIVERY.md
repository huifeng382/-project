...
> ## ⚠ V3.2（2026-09-26，当前交付版）
>
> 针对接收方 `docs/V3_ISSUES.md`（重出规格）做的**结构修复版**，仍未完成全部重出项。
>
> | 项 | V3.0（首版） | **V3.2（本版）** |
> |---|---|---|
> | 电路 / 组 / 行 | 12,455 / 837 / 599,976 | **8,222 / 553 / 435,895** |
> | 组内重复设计 | 40.3%（4,905 个不同 `.tl` 编译出同一网表） | **0**（设计级去重：netlist + 宏体 md5 + cell_types + pins + parasitics，**不含** `transistor_count`） |
> | 组大小 | 名义 10-15、去重后均值 9.35 | **10-15（每个都是互异设计）** |
> | 组内跨度判据①（≥2 个变体比组内最优慢 >10%） | 61.5% | **99.6%**（目标 ≥80%） |
> | 组内跨度中位 | 0.195 | **0.51**（目标 0.30-0.60） |
> | `transistor_count` | 与 netlist 不自洽（15.1% 电路） | **= Σ(DUT X_ 行 × 宏体管数)**，交付内容的纯函数；旧值保留为 `transistor_count_legacy` |
> | 新增列 | — | `depth`、`logic_class_counts_json`、`macro_body_md5`（静态记录可判重、近单射）、`weak_gate_count`、`design_key` |
> | 9x6 占比 | 5,068 电路 / 70.9% 行 | **1,995 电路**（硬限 2,000） |
> | Tier A 行占比 | 37.0% | 42.3%（总行数达 60 万时自动回到 37%） |
>
> **形状交付**：2x2 147、3x2 176、4x3 314、5x2 900、5x5 480、7x4 1090、8x3 780、8x4 1494、
> 9x1 180、9x6 1995、16x1 212、2x1 95、2x3 105、3x1 70、4x1 232、5x1 217、8x1 160；**1x1 = 0**。
>
> **未闭合项（含依据）**
> 1. **约 2,000 个缺口设计不可达**：小/中形状设计空间已榨干 —— tl_opt 全新种子收获 760 个候选只得 **260 个互异设计**；
>    SOP 合成 + 改写产出率 **~5%**（600 次尝试 → 31 个）；1x1 结构上限 ≈30-60（1 输入 1 输出仅 4 个函数）。
>    补齐只能靠**新增语料模板族**或**放宽「10-15 变体/组」**。
> 2. **行数 435,895（规格 ~60 万）**：池内可用电路受逐形状上限约束，余下缺口需新设计。
> 3. **Tier A 42.3%**：配额驱动 Tier A = 184,479 行；总行数补齐后自动落到 37%。
> 4. **P1-3 slew/load 扫描未做**（需接收方定 N，唯一显著增加总算力的项）。
> 5. **P2-4 Tier B 过渡带**：Tier B 内 140-190 管带/深 6-8 带见 `metadata.json`，仍未达 20%/15%。
>
> 以下 V3.0 首版说明保留作对照，**数值以本节为准**。
