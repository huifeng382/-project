...
def main():
    ...
    # ---------- 剔除 DELAY <= 0 的非法样本 ----------
    before_nonpositive = len(dynamic_df)
    dynamic_df = dynamic_df[dynamic_df['DELAY'] > 0]
    removed_nonpositive = before_nonpositive - len(dynamic_df)
    if removed_nonpositive > 0:
        print(f"Removed {removed_nonpositive} rows with DELAY <= 0.")
    print(f"Remaining rows after non-positive removal: {len(dynamic_df)}")
    # ------------------------------------------------------
